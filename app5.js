...
// START OF CHANGE: Add sidebar settings button
...
// START OF CHANGE: Inspector Panel elements
...
// START OF CHANGE: Modified to generate separate <style> tags for the inspector.
...
function applyVibes() {
    try {
        ...
        // START OF CHANGE: Updated script for advanced inspector
...
(function(){
    ...
    function getMatchingCssRules(element) {
        ...
        try {
            for (const sheet of document.styleSheets) {
                if (!sheet.ownerNode || !sheet.ownerNode.dataset.vibeCssNodeId) continue;
        ...
        } catch(e) { console.warn('Could not read cssRules', e); }
        return rules;
    }
    ...
    // --- Part 2: Console and Error Proxy Logic (unchanged from original) ---
    const MAX_DEPTH = 5;
    function serializeArg(arg, depth = 0, seen = new WeakSet()) {
...
// START OF CHANGE: Inspector Panel implementation
function updateStyleInVibeTree(cssNodeId, selector, property, value) {
    ...
    const selectorRegex = new RegExp(`(${safeSelector}[\\s\\n]*\\{[^}]*?\\})`, 'g');
    ...
    cssCode = cssCode.replace(/;(\s*;)+/g, ';').replace(/{\s*;/, '{').replace(/;\s*}/g, '}');
...
function bindEventListeners() {
    ...
    // START OF CHANGE: Add listener for new settings button
