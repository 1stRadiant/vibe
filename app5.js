...
// START OF FIX: Replaced `updateStyleInVibeTree` with a more efficient version.
function updateStyleInVibeTree(cssNodeId, selector, property, value) {
    console.log(`Updating style: ${cssNodeId} -> ${selector} { ${property}: ${value} }`);
    ...
    recordHistory(`CSS edit: ${property}`);
    ...
    cssCode = cssCode.replace(selectorRegex, (match) => {
        ruleUpdated = true;
        // Regex to find the specific property inside the matched block
        const propRegex = new RegExp(`([;\\{\\s]${property}\\s*:\\s*)[^;]+(;?)`, 'i');
        
        if (propRegex.test(match)) {
            return match.replace(propRegex, `$1${value}$2`);
        } else {
    ...
    // Cleanup: remove double semicolons or semicolons before closing braces
    cssCode = cssCode.replace(/;(\s*;)+/g, ';').replace(/;\s*}/g, '}');

    cssNode.code = cssCode.trim();

    // Directly update the style in the iframe for an instant visual refresh
    try {
...
function displayInInspector(data) {
    ...
    inspectorStylesContainer.querySelectorAll('.css-property-name, .css-property-value').forEach(el => {
...
(function(){
    ...
    // START OF FIX: This function now walks up the DOM tree to find all inherited styles.
    function getMatchingCssRules(element) {
        const matchingRules = [];
        const seenSelectors = new Set(); // To avoid adding the same rule multiple times

        let currentEl = element;
        while (currentEl && currentEl.nodeType === 1) { // Loop from the element up to the document root
            try {
                for (const sheet of document.styleSheets) {
                    // Only inspect stylesheets that we've tagged from Vibe
                    if (!sheet.ownerNode || !sheet.ownerNode.dataset.vibeCssNodeId) {
                        continue;
                    }
                    for (const rule of sheet.cssRules) {
                        // Check if the current element in our DOM walk matches the selector
                        if (currentEl.matches(rule.selectorText)) {
                            // Only add a rule if we haven't seen its exact text before
                ...
                // Silently ignore security errors from accessing cross-origin stylesheets
            }
            currentEl = currentEl.parentElement; // Go up to the parent
        }
