...
// START OF CHANGE: Canvas Integration
/**
 * Transforms the hierarchical vibeTree into flat arrays of nodes and connections
 * for the canvas visualization, calculating a default layout if none exists.
 * @param {object} rootNode The root of the vibeTree.
 * @returns {{nodes: Array, connections: Array}}
 */
function transformTreeToCanvasData(rootNode) {
    const nodes = [];
    const connections = [];
    const Y_SPACING = 150;
    const X_SPACING = 200;

    const levelX = new Map(); // Tracks the next available X coordinate for each depth level
    ...
    function traverse(node, parentId, depth) {
        ...
        // If no position is saved, calculate a default one
        if (x === undefined || y === undefined) {
            y = depth * Y_SPACING + 100;
            let currentX = levelX.get(depth) || 0;
            x = currentX;
            levelX.set(depth, currentX + X_SPACING);
        ...
        if (node.children && node.children.length > 0) {
            const childStartX = levelX.get(depth + 1) || 0;
            
            node.children.forEach(child => traverse(child, node.id, depth + 1));
            
            const childEndX = (levelX.get(depth + 1) || X_SPACING) - X_SPACING;
            const childrenMidX = (childStartX + childEndX) / 2;
            
            // If we just calculated the parent's X, adjust it to be centered over its children.
            // If the user already dragged it, we respect the user's position.
            if (node.x === x) { 
                node.x = childrenMidX;
                const existingNode = nodes.find(n => n.id === node.id);
                if (existingNode) existingNode.x = childrenMidX;
            }
        }
    }
...
/**
 * Sends the current vibeTree, transformed for canvas display, to the canvas iframe.
 */
function sendTreeToCanvas() {
    const canvasIframe = document.getElementById('node-canvas-iframe');
    if (!canvasIframe || !canvasIframe.contentWindow) {
        console.warn("Canvas iframe not ready, skipping update.");
        return;
    }
    
    console.log("Sending updated tree to canvas view.");
    const canvasData = transformTreeToCanvasData(vibeTree);
    
    canvasIframe.contentWindow.postMessage({
        type: 'vibe-tree-update',
        payload: canvasData
    }, '*');
}
...
function refreshAllUI() {
    console.log('Refreshing entire UI: Vibe Editor, Website Preview, Full Code, and Canvas.');
...
window.addEventListener('message', (event) => {
    ...
    switch (data.type) {
        ...
        case 'vibe-node-move':
            ...
            break;
        case 'vibe-node-move-end':
            if (data.payload && data.payload.nodeId) {
                const node = findNodeById(data.payload.nodeId);
                if (node) {
                    node.x = data.payload.x;
                    node.y = data.payload.y;
                    autoSaveProject(); // Save the new layout position
                }
            }
            break;
...
async function handleManualSave() {
    ...
    try {
        ...
        console.log(`Project '${currentProjectId}' save request sent and completed.`);
...
function autoSaveProject() {
  ...
  return api.saveProject(currentUser.userId, currentProjectId, vibeTree);
}
