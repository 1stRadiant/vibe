...
function updateStyleInVibeTree(cssNodeId, selector, property, value) {
    console.log(`Persisting style change: ${cssNodeId} -> ${selector} { ${property}: ${value} }`);
    ...
    // Regex to find the selector block. This is a bit naive but works for simple cases.
    // It handles selectors on multiple lines and comments before the opening brace.
    ...
    cssCode = cssCode.replace(selectorRegex, (match) => {
        ...
        if (propRegex.test(match)) {
            ...
            // Property does not exist, add it before the closing brace
            return match.replace(/(\s*})$/, `; ${property}: ${value};\n$1`);
        }
    });

    if (!ruleUpdated) {
        // If the selector block itself wasn't found, add it to the end of the file.
        cssCode += `\n\n${selector} {\n    ${property}: ${value};\n}`;
    }
