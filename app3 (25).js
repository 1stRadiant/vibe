...
function updateDynamicJsVariable(control, triggerRender = true) {
    ...
    const patterns = [
        new RegExp(`(const|let|var)\\s+(${variableName})\\s*=\\s*([^;\\n]+)`),
    ...
    if (isNaN(newValue) || valueStr.trim() === '') {
        replacementValue = `"${valueStr.replace(/"/g, '\\"')}"`;
    } else {
        replacementValue = valueStr;
    }
    ...
    for (let i = 0; i < patterns.length && !matched; i++) {
        ...
        if (pattern.test(targetNode.code)) {
            if (i === 0) {
                ...
                newCode = targetNode.code.replace(pattern, `${variableName} = ${replacementValue}`);
            }
    ...
    if (!matched) {
        console.error(`Could not find variable "${variableName}" to update in node "${targetNode.id}".`);
        return;
    }
