...
// START OF CHANGE: Add elements for Vibe project JSON upload
const uploadVibeButton = document.getElementById('upload-vibe-button');
...
// START OF CHANGE: Add handler for uploading a Vibe project JSON file directly
/**
 * Handles uploading a .json file containing a vibeTree, loading it directly
 * into the application state without any AI processing or decomposition.
 */
async function handleVibeProjectUpload() {
    const file = vibeFileInput.files[0];
    if (!file) {
        alert("Please select a Vibe project (.json) file to upload.");
        return;
    }

    console.log(`Vibe project file selected: ${file.name} (${file.type})`);
    if (file.type && !file.type.includes('json')) {
        console.warn(`Warning: Selected file is not application/json. Proceeding anyway.`);
    }

    const button = uploadVibeButton;
    ...
    reader.onload = async (event) => {
        try {
            const fileContent = event.target.result;
            const newTree = JSON.parse(fileContent);

            // Validate the uploaded JSON to ensure it's a valid vibeTree
            if (!newTree || typeof newTree !== 'object' || newTree.id !== 'whole-page' || newTree.type !== 'container' || !Array.isArray(newTree.children)) {
                throw new Error("Invalid Vibe project file format. The file must contain a valid vibeTree JSON object.");
            }

            // Assign a new, unique project ID for local storage
            const baseId = file.name.replace(/\.json$/i, '').toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9\-]/g, '');
            let projectId = baseId || `vibe-project-${Date.now()}`;
            currentProjectStorageType = 'local';
            const existing = await localApi.listProjects();
            let suffix = 1;
            while (existing.includes(projectId)) {
            ...
            // Directly load the tree without processing
            vibeTree = newTree;

            // Update application state and UI
            updateSaveButtonStates();
            if(shareProjectButton) shareProjectButton.disabled = true; // It's a local project
            if(openAiStructureModalButton) openAiStructureModalButton.disabled = true;

            resetHistory();
            refreshAllUI();
            switchToTab('preview');
            autoSaveProject(); // Save the new project to local storage
            console.log(`Vibe project '${currentProjectId}' loaded directly from JSON file.`);

        } catch (error) {
            console.error("Error loading Vibe project file:", error);
            alert(`An error occurred while loading the project file: ${error.message}`);
        } finally {
            button.disabled = false;
            button.innerHTML = originalText;
            vibeFileInput.value = ''; // Allow re-uploading the same file
        }
    };
...
function selectComponentForPreview(componentId) {
    ...
    // START OF CHANGE: Add event listeners for Vibe project upload
    if (uploadVibeButton) uploadVibeButton.addEventListener('click', () => vibeFileInput.click());
    if (vibeFileInput) vibeFileInput.addEventListener('change', handleVibeProjectUpload);
    // END OF CHANGE
...
If the user says "control the particle count" and you find a node with id "effect-script" containing \`const PARTICLE_COUNT = 100;\`, your output should be:
{
  ...
  "targetJsVariable": "PARTICLE_COUNT"
        ...
        // Add the full vibeTree to the user prompt for context
        ...
        refreshAllUI();
        // Initial application of styles/JS values
...
function handleControlChange(event) {
    ...
    // NEW: Differentiate between CSS and JS controls
    if (control.controlTarget === 'js') {
        updateDynamicJsVariable(control);
    } else {
...
function updateDynamicJsVariable(control) {
    ...
    // Enhanced regex patterns to handle various declaration formats
    const patterns = [
        // Standard declarations: const/let/var VARIABLE = value;
        new RegExp(`(const|let|var)\\s+(${variableName})\\s*=\\s*([^;]+);`, 'g'),
        // Object property: VARIABLE: value, or VARIABLE: value }
        new RegExp(`(${variableName})\\s*:\\s*([^,}]+)([,}])`, 'g'),
        // Assignment without declaration: VARIABLE = value;
        new RegExp(`(${variableName})\\s*=\\s*([^;]+);`, 'g')
    ];
    ...
    // Determine if the new value should be quoted
    ...
    if (targetNode.code !== newCode) {
        ...
        // Re-render the entire preview to execute the updated script
        applyVibes();
        autoSaveProject();
    }
}
