...
function generateFullCodeString(tree) {
    ...
    // START OF CHANGE: Generate individual style tags for inspector targeting
    let styleTags = '';
    // END OF CHANGE
    ...
    function traverseAndCollect(node) {
        switch (node.type) {
            case 'head':
                ...
                break;
            // START OF CHANGE: Collect CSS into separate style tags
            case 'css':
                styleTags += `<style data-vibe-css-node-id="${node.id}">\n${node.code}\n</style>\n`;
                break;
            // END OF CHANGE
            case 'javascript':
            case 'js-function':
...
let vibeTree = {};
// START OF CHANGE: Add state for CSS inspector
let inspectedElementState = {
    element: null,
    rules: []
};
// END OF CHANGE
...
async function handleZipUpload() {
    ...
    try {
        ...
        vibeTree = newTree;
        await autoSaveProject(); // Save to backend
...
(function(){
    ...
    document.addEventListener('click',e=>{if(!inspectEnabled)return;const nodeIdInfo=getNodeId(e.target);if(nodeIdInfo){e.preventDefault();e.stopPropagation();const targetEl=nodeIdInfo.element;const appliedRules=[];const elementInfo={tag:targetEl.tagName.toLowerCase(),id:targetEl.id,classes:Array.from(targetEl.classList)};for(const sheet of document.styleSheets){try{if(!sheet.cssRules)continue;const sourceNodeId=sheet.ownerNode.dataset.vibeCssNodeId;if(!sourceNodeId)continue;for(const rule of sheet.cssRules){if(targetEl.matches(rule.selectorText)){const styleProps={};for(let i=0;i<rule.style.length;i++){const propName=rule.style[i];styleProps[propName]=rule.style.getPropertyValue(propName)}appliedRules.push({selector:rule.selectorText,properties:styleProps,sourceNodeId:sourceNodeId})}}}catch(err){}}window.parent.postMessage({type:'vibe-inspect-data',payload:{element:elementInfo,rules:appliedRules}},'*');if(hoverEl)hoverEl.classList.remove('__vibe-inspect-highlight-hover');const el=nodeIdInfo.element;el.classList.add('__vibe-inspect-highlight-clicked');setTimeout(()=>el.classList.remove('__vibe-inspect-highlight-clicked'),200)}},true);
    ...
    // --- Part 3: Message listener for toggling inspect and updating styles ---
    window.addEventListener('message',e=>{const data=e.data||{};if(data.type==='toggle-inspect'){inspectEnabled=data.enabled;if(inspectEnabled)ensureStyles();else{if(hoverEl){hoverEl.classList.remove('__vibe-inspect-highlight-hover');hoverEl=null}window.parent.postMessage({type:'vibe-inspect-disable'},'*')}}else if(data.type==='vibe-update-style'){const{sourceNodeId,newCss}=data;const styleTag=document.querySelector(\`style[data-vibe-css-node-id="\${sourceNodeId}"]\`);if(styleTag){styleTag.textContent=newCss}}});
})();
<\/script>`;
...
// START OF FIX: Inspector Panel Logic
let inspectEnabled = false;
...
function toggleInspectMode() {
    ...
    if (inspectorPanel) {
        if (inspectEnabled) {
            ...
            // Add a placeholder message until an element is clicked
            inspectorElementInfo.innerHTML = `<div>Select an element to inspect.</div>`;
...
// Render the inspector panel with data from the iframe
function renderInspectorPanel() {
    if (!inspectorPanel || !inspectedElementState.element) return;

    inspectorPanel.classList.add('visible');

    const { tag, id, classes } = inspectedElementState.element;
    inspectorElementInfo.innerHTML = `
        <span style="color: #e06c75;">&lt;${tag}</span>
        ${id ? `<span style="color: #61afef;">#${id}</span>` : ''}
        ${classes.length > 0 ? `<span style="color: #98c379;">.${classes.join('.')}</span>` : ''}
        <span style="color: #e06c75;">&gt;</span>
    `;

    inspectorStylesContainer.innerHTML = '';
    inspectedElementState.rules.forEach(rule => {
        const ruleBlock = document.createElement('div');
        ruleBlock.className = 'css-rule-block';
        ruleBlock.dataset.sourceNodeId = rule.sourceNodeId;

        ruleBlock.innerHTML = `
            <div class="css-rule-header">
                <span class="css-rule-selector">${rule.selector}</span>
                <span class="css-rule-source" title="Go to source node">${rule.sourceNodeId}</span>
            </div>
            <div class="css-properties"></div>
        `;
        
        const propertiesContainer = ruleBlock.querySelector('.css-properties');
        for (const [propName, propValue] of Object.entries(rule.properties)) {
            const propEl = document.createElement('div');
            propEl.className = 'css-property';
            propEl.innerHTML = `
                <input type="checkbox" class="css-property-toggle" checked>
                <span class="css-property-name" contenteditable="true">${propName}</span>:&nbsp;
                <span class="css-property-value" contenteditable="true">${propValue}</span>;
            `;
            propertiesContainer.appendChild(propEl);
        }

        const addPropButton = document.createElement('button');
        addPropButton.textContent = '+ Add Property';
        addPropButton.className = 'action-button add-property-button';
        propertiesContainer.appendChild(addPropButton);

        inspectorStylesContainer.appendChild(ruleBlock);
    });
}

// Handles edits from the inspector panel and updates the tree + preview
function handleInspectorEdit(element) {
    const ruleBlock = element.closest('.css-rule-block');
    if (!ruleBlock) return;

    const sourceNodeId = ruleBlock.dataset.sourceNodeId;
    const selector = ruleBlock.querySelector('.css-rule-selector').textContent;

    let propertiesString = '';
    ruleBlock.querySelectorAll('.css-property').forEach(propEl => {
        const isEnabled = propEl.querySelector('.css-property-toggle').checked;
        const propName = propEl.querySelector('.css-property-name').textContent.trim();
        const propValue = propEl.querySelector('.css-property-value').textContent.trim();
        
        propEl.classList.toggle('disabled', !isEnabled);
        
        if (isEnabled && propName && propValue) {
            propertiesString += `  ${propName}: ${propValue};\n`;
        }
    });

    const newRuleCss = `${selector} {\n${propertiesString}}`;
    const nodeToUpdate = findNodeById(sourceNodeId);
    if (!nodeToUpdate) return;
    
    // Replace the specific rule within the node's code
    const escapedSelector = selector.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    const ruleRegex = new RegExp(`${escapedSelector}\\s*\\{[^}]*\\}`, 'g');
    
    let newFullCss;
    if (nodeToUpdate.code.match(ruleRegex)) {
        newFullCss = nodeToUpdate.code.replace(ruleRegex, newRuleCss);
    } else {
        newFullCss = nodeToUpdate.code + '\n\n' + newRuleCss;
    }

    if (nodeToUpdate.code !== newFullCss) {
         recordHistory(`Inspector CSS edit for ${sourceNodeId}`);
         nodeToUpdate.code = newFullCss;
         autoSaveProject();
    }
    
    previewContainer.contentWindow.postMessage({
...
function addInspectorEventListeners() {
    // Event delegation for dynamic content
    inspectorStylesContainer.addEventListener('blur', (event) => {
        if (event.target.matches('[contenteditable="true"]')) {
            handleInspectorEdit(event.target);
        }
    }, true); // Use capture phase

    inspectorStylesContainer.addEventListener('change', (event) => {
        if (event.target.matches('.css-property-toggle')) {
            handleInspectorEdit(event.target);
        }
    });
    
    inspectorStylesContainer.addEventListener('click', (event) => {
        if (event.target.matches('.css-rule-source')) {
            const nodeId = event.target.closest('.css-rule-block').dataset.sourceNodeId;
            switchToTab('editor');
            const nodeEl = editorContainer.querySelector(`.vibe-node[data-node-id="${nodeId}"]`);
            if (nodeEl) {
        ...
        } else if (event.target.matches('.add-property-button')) {
            const propertiesContainer = event.target.closest('.css-properties');
            const propEl = document.createElement('div');
            propEl.className = 'css-property';
            propEl.innerHTML = `
               <input type="checkbox" class="css-property-toggle" checked>
            ...
            `;
            propertiesContainer.insertBefore(propEl, event.target);
            propEl.querySelector('.css-property-name').focus();
        }
    });
...
window.addEventListener('message', (event) => {
    ...
    switch (data.type) {
        case 'vibe-node-click':
            if (data.nodeId) {
                // This message is now deprecated in favor of vibe-inspect-data,
                // but we can keep it as a fallback for opening the edit modal.
                if (!inspectEnabled) {
                    openEditNodeModal(data.nodeId);
                }
            }
            break;
        // START OF CHANGE: New message handlers for inspector
        case 'vibe-inspect-data':
            if (data.payload) {
                inspectedElementState = data.payload;
                renderInspectorPanel();
            }
            break;
        case 'vibe-inspect-disable':
            if (inspectorPanel) {
                inspectorPanel.classList.remove('visible');
            }
            break;
...
async function handleManualSave() {
    ...
    if (manualSaveButton.dataset.saving === 'true') {
        return;
    }
    manualSaveButton.dataset.saving = 'true';
    ...
    try {
        ...
        console.log(`Project '${currentProjectId}' saved successfully.`);
        ...
        setTimeout(() => {
            ...
            delete manualSaveButton.dataset.saving;
        }, 2000);
        ...
        // Revert after showing error state
        setTimeout(() => {
            manualSaveButton.innerHTML = originalHtml;
            manualSaveButton.disabled = false;
            delete manualSaveButton.dataset.saving;
...
async function autoSaveProject() {
  ...
  try {
    const savePromise = api.saveProject(currentUser.userId, currentProjectId, vibeTree);
...
function initMainApp() {
    ...
    initializeMermaid();
    bindEventListeners();
    addInspectorEventListeners(); // Add new event listeners for the inspector
    checkLoggedInState();
}
