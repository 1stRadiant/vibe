...
1. Automatic saving of form data for any <form> with the 'data-vibe-form' attribute.
2. A window.vibe.loadData() function to retrieve and display that data.

HOW TO USE:
--------------------------------------------------------------------------------
1. To Save Form Data:
   - Add the 'data-vibe-form' attribute to your <form> tag.
   - The value of the attribute is a unique name for your form, e.g., "contact-form".
   - Example: <form data-vibe-form="contact-form"> ... </form>
   - The script will automatically capture submissions, save them to the database,
...
2. To Display Saved Data:
   ...
   - This function returns a Promise. Use .then() to process the data.
   - Example JavaScript:
     ------------------
     async function displaySubmissions() {
         try {
             ...
             submissions.forEach(item => {
                 ...
                 // Assuming form had 'name' and 'email' fields
                 const name = item.data.name || 'N/A';
                 const email = item.data.email || 'N/A';
                 const date = new Date(item.timestamp).toLocaleString();
                 li.textContent = \`Name: \${name}, Email: \${email} (Submitted on: \${date})\`;
                 list.appendChild(li);
             });
...
*/
(function() {
    const APPS_SCRIPT_URL = '${api.APPS_SCRIPT_URL}'; // Injected from parent app
    ...
    function jsonpRequest(action, params = {}) {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = () => {
                delete window[callbackName];
                document.body.removeChild(script);
                reject(new Error('API request failed. Check network connection and script URL.'));
            };
    ...
    async function saveFormData(formId, formData) {
        ...
        return jsonpRequest('saveFormData', { userId: USER_ID, projectId: PROJECT_ID, formId, data: dataString });
    }

    async function loadFormData(formId) {
        return jsonpRequest('loadFormData', { userId: USER_ID, projectId: PROJECT_ID, formId });
    }
    ...
    window.vibe = {
        loadData: function(formId) {
            if (!formId) {
                console.error('vibe.loadData requires a formId.');
    ...
    document.addEventListener('submit', async (e) => {
        ...
        if (!formId) return; // Not a vibe-managed form
        ...
        if (submitButton) {
            ...
            if(submitButton.innerHTML) { submitButton.innerHTML = 'Submitting...'; } else { submitButton.value = 'Submitting...'; }
        }
        ...
        try {
            ...
            if(submitButton) {
                ...
                if(submitButton.innerHTML) { submitButton.innerHTML = originalButtonContent; } else { submitButton.value = originalButtonContent; }
            }
        }
    }, true);
})();
...
async function handleManualSave() {
    ...
    try {
        // Since api.saveProject is now a fire-and-forget queue, we can't `await` it.
        // We just trigger it. The UI feedback will be optimistic.
...
async function processContextUpload(event) {
    // START OF FIX: Correctly access the single file from the FileList object.
