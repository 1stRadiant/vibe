...
${htmlContent.trim()}
    ...
    <script id="vibe-project-script">(function() {${jsContent.trim()}})();<\/script>
</body>
</html>`;
}
...
// START OF CHANGE: Replaced manual save with specific cloud/local save buttons
...
// START OF CHANGE: Add Local Storage UI Elements
...
// START: Elements for AI-prompted controls modal
...
// END: Elements for AI-prompted controls modal
...
let controlChangeDebounceTimer = null; // For debouncing control input

let currentProjectId = null;
let currentProjectStorageType = 'cloud'; // 'cloud' or 'local'
...
function handleLogout() {
    ...
    updateSaveButtonStates(); // NEW: Correctly disables cloud save
...
// START OF CHANGE: New functions for saving to cloud or local storage
...
async function handleSaveToCloud() {
    ...
    // If the current project isn't a cloud project, we're saving a copy, so get a new ID.
    if (currentProjectStorageType !== 'cloud') {
        const newCloudId = await getNewProjectId('cloud', currentProjectId);
        if (newCloudId === null) return; // User canceled the prompt
    ...
    try {
        ...
        // On success, update the application state to reflect the new primary location
        ...
        // Switch the project list UI to 'cloud' to show the new project
...
async function handleSaveToLocal() {
    ...
    let projectIdToSave = currentProjectId;
    // If the current project isn't a local project, we're saving a copy, so get a new ID.
    if (currentProjectStorageType !== 'local') {
        const newLocalId = await getNewProjectId('local', currentProjectId);
        if (newLocalId === null) return; // User canceled the prompt
    ...
    try {
        ...
        // Switch the project list UI to 'local' to show the new project
...
`;
        ...
        // Use the existing function to process the new code and refresh everything
        await processCodeAndRefreshUI(newFullCode);

        // After refresh, the full code editor is updated. We can try to restore the view.
        // A full selection restoration is complex, but scrolling to the original area is a good UX improvement.
        ...
        fullCodeAiPromptInput.value = ''; // Clear prompt on success
...
async function handleFileUpload() {
    ...
    // For file uploads, default to local storage but check for conflicts.
    currentProjectStorageType = 'local';
    ...
    updateSaveButtonStates();
    if(openAiStructureModalButton) openAiStructureModalButton.disabled = (currentProjectStorageType !== 'cloud');
    if(shareProjectButton) shareProjectButton.disabled = (currentProjectStorageType !== 'cloud');
    ...
    reader.onload = async (event) => {
        ...
        autoSaveProject();
        console.log(`HTML project '${currentProjectId}' imported locally and processed.`);
    };
...
async function handleZipUpload() {
    ...
    try {
        ...
        // ZIPs are also imported locally by default
        currentProjectStorageType = 'local';
        const existing = await localApi.listProjects();
        let suffix = 1;
        while (existing.includes(projectId)) {
        ...
        vibeTree = newTree;
        autoSaveProject(); // Save to local storage

        resetHistory();
        refreshAllUI();
        switchToTab('preview');
        console.log(`ZIP project '${currentProjectId}' imported locally successfully.`);
...
function renderTaskQueue() {
    ...
    // Add delegated event listener once
...
async function handleFixError(errorMessage, fixButton) {
    ...
    // Create a new task to fix the error and add it to the queue
    const fixTask = `A runtime error was detected. Analyze the error and the code to fix it. Error details: ${errorMessage}`;
    taskQueue.unshift(fixTask); // Add to the front of the queue
    renderTaskQueue();
    updateTaskQueueUI();
...
`;
        ...
        // The magic happens here: process the new code and refresh everything
        await processCodeAndRefreshUI(newFullCode);
...
function switchToTab(tabId) {
    ...
    if (tabId === 'agent') {
        ...
        updateTaskQueueUI(); // Ensure UI is correct on tab switch
    }
}
...
// --- START OF CHANGE: Local Project Persistence using IndexedDB ---
...
function autoSaveProject() {
    if (!currentProjectId || !vibeTree) return;
    if (currentProjectStorageType === 'cloud' && !currentUser) return;
...
async function handleExecuteGenerateControls() {
    ...
    try {
        const systemPrompt = `You are an expert UI/UX designer and full-stack developer. Your task is to make a web component editable by creating UI controls AND the underlying code modifications to make them work.

**INPUT:**
1. **User Request:** A plain text description of the desired controls (e.g., "controls to move the shapes").
2. **Node Context:** The JSON object of the Vibe Tree node the controls will apply to.
3. **Full Vibe Tree:** The entire project structure for context.

**TASK:**
1.  **Design the Controls:** Decide what controls (sliders, color pickers) are needed. For variables you want to control, they should be declared with \`let\`, not \`const\`.
2.  **Implement the Logic:** Determine what code changes (CSS or JavaScript) are required to make these controls functional. This might involve adding new CSS properties, creating new JavaScript variables, or writing new functions that read these variables and update the DOM.
3.  **Generate the Output:** Produce a single JSON object containing both the control definitions and the necessary code changes.

**OUTPUT (MANDATORY JSON SCHEMA):**
You MUST respond with a single, valid JSON object with two top-level keys: \`controls\` and \`actions\`. Do not add any other text.
{
  "controls": [
    {
      "id": "position-x", "label": "Horizontal Position", "type": "slider", "value": 50, "min": 0, "max": 100, "unit": "px",
      "controlTarget": "js", "targetJsNodeId": "shape-logic-script", "targetJsVariable": "shapeX"
    }
  ],
  "actions": [
    {
      "actionType": "update",
      "nodeId": "shape-logic-script",
      "newDescription": "Script to manage shape position, now controlled by UI.",
      "newCode": "let shapeX = 50; let shapeY = 50; \\nfunction updateShapePosition() { \\n const shape = document.getElementById('my-shape'); \\n if (shape) { \\n  shape.style.transform = \`translate(\${shapeX}px, \${shapeY}px)\`; \\n } \\n} \\n// We need a way to call this function whenever the variables change. The app handles this."
    }
  ]
}

**RULES & ANALYSIS:**
- The \`controls\` array defines the UI. The \`actions\` array makes the UI functional. They must work together.
- If you define a JS control for a variable \`shapeX\`, you MUST provide an \`action\` that creates or updates a script to USE \`shapeX\` to modify the DOM (e.g., setting a \`style.transform\`).
- The \`targetJsNodeId\` in your control definition must match a \`nodeId\` in your actions or an existing node in the tree.
- The \`actions\` can modify ANY node in the tree, not just the one the controls are attached to. This is essential for separating concerns (e.g., updating a global CSS or JS file).`;
        ...
        if (!aiResponse || !Array.isArray(aiResponse.controls) || !Array.isArray(aiResponse.actions)) {
            throw new Error("AI did not return a valid response object with 'controls' and 'actions' arrays.");
        }
        
        const { controls: newControls, actions: agentActions } = aiResponse;

        recordHistory(`Generate controls and logic for ${nodeId}`);

        // Part 1: Execute the code modification actions to implement the logic
        if (agentActions.length > 0) {
            console.log(`Executing ${agentActions.length} actions from control generation.`);
            executeAgentPlan({ plan: "Implementing logic for new UI controls.", actions: agentActions }, (msg, type) => console.log(`[ControlsAgent] ${msg}`));
        }
        
        // Part 2: Merge the new controls into the node
        // We must re-find the node in case the agent actions modified it or its parents
        const targetNode = findNodeById(nodeId); 
        if (!targetNode) {
            throw new Error(`Node ${nodeId} may have been modified or removed by the AI. Cannot attach controls.`);
        }

        const existingControls = new Map((targetNode.controls || []).map(c => [c.id, c]));
        newControls.forEach(c => existingControls.set(c.id, c));
        targetNode.controls = Array.from(existingControls.values());

        console.log(`Generated and merged ${newControls.length} controls for node ${nodeId}`);

        // Part 3: Refresh UI (already done by executeAgentPlan) and apply initial values
        
        // After refresh, apply the initial values. The logic should now exist in the code.
        newControls.forEach(control => {
            if (control.controlTarget === 'js') {
                updateDynamicJsVariable(control, false); // Update state without re-rendering yet
            }
        });
        updateDynamicStyles(false); // Update state without re-rendering yet

        // Finally, trigger a single, efficient re-render/re-execution
        reExecuteAllScripts();
...
function handleControlChange(event) {
    ...
    // Instant UI feedback for sliders
    if (input.type === 'range') {
        const node = findNodeById(nodeId);
        const control = node.controls.find(c => c.id === controlId);
        const valueSpan = input.nextElementSibling;
        if (valueSpan && valueSpan.classList.contains('slider-value')) {
            valueSpan.textContent = `${input.value}${control.unit || ''}`;
        }
    }

    // Debounce the expensive update operation
    clearTimeout(controlChangeDebounceTimer);
    controlChangeDebounceTimer = setTimeout(() => {
        const node = findNodeById(nodeId);
...
/**
 * Re-executes all project scripts in the preview iframe.
 * This is faster than a full reload and is used for dynamic JS updates.
 */
function reExecuteAllScripts() {
    try {
        const doc = previewContainer.contentWindow.document;
        
        const oldScript = doc.getElementById('vibe-project-script');
        if (oldScript) {
            oldScript.remove();
        }
        ...
        let jsContent = '';
        function traverse(node) {
            if (['javascript', 'js-function', 'declaration'].includes(node.type) && node.code) {
                jsContent += node.code + '\n\n';
            }
            if (node.children) node.children.forEach(traverse);
        }
        ...
        if (jsContent.trim()) {
            ...
            newScript.id = 'vibe-project-script';
            newScript.textContent = `(function() { try { ${jsContent.trim()} } catch(e) { console.error('Dynamic Script Error:', e); } })();`;
            doc.body.appendChild(newScript);
        }
        console.log("Re-executed project scripts dynamically.");
    } catch (e) {
        console.error("Failed to re-execute scripts, falling back to full refresh.", e);
        applyVibes(); // Fallback to the slower but robust full refresh
    }
}
 ...
 */
function updateDynamicJsVariable(control, triggerRender = true) {
    ...
    // A more robust regex that handles let/var/const and optional semicolons/spacing.
    const pattern = new RegExp(`(let|var|const)\\s+(${variableName})\\s*=\\s*[^;\\n]+;?`);
    ...
    // Check if the value is a number, but not an empty string which would be parsed as 0.
    if (!isNaN(parseFloat(valueStr)) && isFinite(newValue) && valueStr.trim() !== '') {
        replacementValue = valueStr; // It's a number, use it directly
    } else {
        // It's a string, so enclose in quotes and escape internal quotes
        replacementValue = `"${valueStr.replace(/"/g, '\\"')}"`;
    }
    
    if (pattern.test(targetNode.code)) {
        newCode = targetNode.code.replace(pattern, `$1 ${variableName} = ${replacementValue};`);
    ...
    if (!matched) {
        console.error(`Could not find declaration for variable "${variableName}" to update in node "${targetNode.id}". Pattern: ${pattern}`);
        return;
    }
    ...
    if (targetNode.code !== newCode) {
        ...
        recordHistory(`Control: Set ${variableName} to ${newValue}`);
...
/**
 * Updates the dynamic stylesheet in the preview iframe.
 * @param {boolean} [triggerRender=true] - Whether to trigger a re-render.
 */
function updateDynamicStyles(triggerRender = true) {
    ...
    if (stylesNode.code !== newCssCode) {
        ...
        if(triggerRender) {
             try {
                const doc = previewContainer.contentWindow.document;
                let styleEl = doc.getElementById('vibe-dynamic-styles-in-iframe');
                if (!styleEl) {
                    styleEl = doc.createElement('style');
                    styleEl.id = 'vibe-dynamic-styles-in-iframe';
                    doc.head.appendChild(styleEl);
                }
                styleEl.textContent = newCssCode;
                console.log("Injected dynamic styles into preview.");
            } catch (e) {
                console.error("Failed to inject dynamic styles, falling back to full refresh.", e);
                applyVibes(); // Fallback if iframe is not accessible
            }
        }
        autoSaveProject();
    }
...
function bindEventListeners() {
    ...
    // AI Structure Update modal listeners
    ...
    // AI Project Edit modal listeners
...
async function handleGenerateProject() {
    try {
        ...
        // Activate the correct tab in the project list
...
function handleNodeContentToggle(event) {
    ...
    // FIX: Do not toggle the node's visibility if the click originated on any
    // interactive element like a button, link, or input. This prevents this
    // generic handler from interfering with more specific controls within the node.
