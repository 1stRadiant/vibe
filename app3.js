...
function refreshAllUI() {
    ...
    // Notify APUI bridge subscribers (e.g. the APUI visual editor iframe)
    if (window.__vibeBridge?._refreshCallbacks?.length) {
        window.__vibeBridge._refreshCallbacks.forEach(cb => { try { cb(vibeTree); } catch(e) {} });
    }
    // Also poke the APUI iframe directly if it's loaded and has refreshFromTree
    try {
        const frame = document.getElementById('apui-frame');
        if (frame?.contentWindow?.refreshFromTree) frame.contentWindow.refreshFromTree();
    } catch(e) {}
}
