...
1.  **Component Node Object:** Each object in the array must have:
    ...
    *   **JAVASCRIPT MODULARITY (CRITICAL):**
        *   **Do NOT create monolithic script nodes.**
        *   **Functions:** Every standalone function MUST be its own node with \`type: "js-function"\`. ID format: \`function-{action-name}\`.
        *   **Variables:** Group related constants/variables into \`type: "declaration"\` nodes. ID format: \`declaration-{group-name}\`.
        *   **Event Listeners:** Distinct logic blocks (like adding an event listener) should be separate \`javascript\` nodes. ID format: \`setup-{element}-listeners\`.
        *   **Separation of Concerns:** Keep logic separate so it can be targeted individually for updates.
`;
...
**JAVASCRIPT DECOMPOSITION (CRITICAL - DO NOT CLUSTER):**
    *   **Goal:** Atomic, modular nodes.
    *   **Functions:** Extract EVERY named function into its own \`js-function\` node. ID: \`function-{name}\`.
    *   **Variables:** Extract top-level constants/vars into \`declaration\` nodes. ID: \`declaration-{name}\`.
    *   **Listeners/Logic:** Do NOT dump remaining code into one big "global-scripts" node. Split it!
        *   If there is a block setting up a nav menu, make a \`javascript\` node \`setup-nav\`.
        *   If there is a block for form validation, make a \`javascript\` node \`setup-form-validation\`.`;
...
**JAVASCRIPT RULES:**
- **Atomic Nodes:** Never create a single monolithic script. Split logic into small, targeted nodes (e.g., \`function-toggle-modal\`, \`setup-form-listeners\`, \`declaration-config\`).
- **Targeting:** When updating logic, find the specific node responsible for that logic. Do not rewrite the whole script file if you can update just one function node.
