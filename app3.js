...
function decompressProjectData(dataString) {
    try {
        if (!dataString) throw new Error("Input string is empty");
...
function onLoginSuccess(userData) {
    ...
    populateProjectList();
    // Don't auto-reset if we are restoring a session
    // resetToStartPage();
...
function switchToTab(tabId) {
    ...
    // Save state
    localStorage.setItem(SESSION_KEYS.TAB, tabId);
...
function clearSessionMetadata() {
    localStorage.removeItem(SESSION_KEYS.PROJECT_ID);
    localStorage.removeItem(SESSION_KEYS.STORAGE_TYPE);
    localStorage.removeItem(SESSION_KEYS.GITHUB_SOURCE);
    // Note: We deliberately keep the TAB preference so the user stays on the same screen
}

async function restoreSession() {
    // 1. Restore Tab Preference
    const lastTab = localStorage.getItem(SESSION_KEYS.TAB);
    
    // 2. Check for saved project
    const pid = localStorage.getItem(SESSION_KEYS.PROJECT_ID);
    const type = localStorage.getItem(SESSION_KEYS.STORAGE_TYPE);

    // If no project was saved, just handle the tab
    if (!pid) {
    ...
    try {
        ...
        if (type === 'cloud') {
            // Check if we are actually logged in before trying to fetch cloud data
            // If checkLoggedInState hasn't finished, wait a moment, or rely on currentUser variable
            if (!currentUser) {
                console.warn("Restore aborted: Cloud project detected but no user logged in. Waiting for auth or manual login.");
                clearSessionMetadata(); // Safe approach: clear session so we don't loop errors. User can load manually.
            ...
            try {
                ...
                console.error("Cloud restore failed:", cloudErr);
                throw new Error("Could not fetch project from cloud.");
            }
        } 
        else if (type === 'local') {
            try {
                ...
                console.error("Local restore failed:", localErr);
                throw new Error("Could not fetch project from local DB.");
            }
        } 
        else if (type === 'github') {
            const sourceStr = localStorage.getItem(SESSION_KEYS.GITHUB_SOURCE);
            if (sourceStr && sessionStorage.getItem('githubToken')) {
                ...
                if (content) {
                 ...
                 throw new Error("GitHub token or source missing for session restore.");
            }
        }
        ...
        // Critical: Clear metadata to prevent infinite reload loop on corrupt data
        clearSessionMetadata(); 
        
        // Return to start screen
        resetToStartPage();
        alert("Previous session could not be restored. Returning to Start.");
    }
}
