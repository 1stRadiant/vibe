...
function applyVibes() {
    try {
        ...
        // Expanded inspector script to handle dragging
        const commsScriptText = `
<script>
(function(){
    // --- Part 1: Inspector & Drag Logic ---
    ...
    const inspectorStyles = \`
        .__vibe-inspect-highlight-hover {
            ...
            box-shadow: 0 0 8px rgba(229,192,123,.8) !important;
            cursor: grab;
        }
        .__vibe-inspect-highlight-clicked {
            outline: 3px solid #61afef !important;
            outline-offset: 2px !important;
            box-shadow: 0 0 12px rgba(97,175,239,.9) !important;
            transition: all .5s ease-out !important;
        }
        .__vibe-drop-placeholder {
            height: 4px;
    ...
    function getNodeId(el){
        const c = el.closest('[data-vibe-node-id]');
        return c ? { nodeId: c.dataset.vibeNodeId, element: c } : null;
    }

    function ensureStyles(){
        if(document.getElementById('vibe-inspector-styles')) return;
        const s = document.createElement('style');
        s.id = 'vibe-inspector-styles';
        s.textContent = inspectorStyles;
        document.head.appendChild(s);
    }

    function createPlaceholder() {
        const p = document.createElement('div');
        p.className = '__vibe-drop-placeholder';
        return p;
    }
    ...
    document.addEventListener('mouseover', e => {
        ...
        if (t) {
            if (hoverEl && hoverEl !== t.element) hoverEl.classList.remove('__vibe-inspect-highlight-hover');
            hoverEl = t.element;
    ...
    document.addEventListener('mouseout', e => {
        if (hoverEl && !hoverEl.contains(e.relatedTarget) && !isDragging) {
            hoverEl.classList.remove('__vibe-inspect-highlight-hover');
            hoverEl = null;
        }
    });
    ...
    document.addEventListener('mousemove', e => {
        ...
        if (!isDragging) {
            ...
            if (Math.abs(e.clientX - startX) > 5 || Math.abs(e.clientY - startY) > 5) {
                ...
                dragClone.style.height = dragSrcEl.offsetHeight + 'px';
                ...
                dragClone.style.boxShadow = '0 10px 20px rgba(0,0,0,0.2)';
                document.body.appendChild(dragClone);

                dropPlaceholder = createPlaceholder();
        ...
        if (isDragging) {
            ...
            const targetObj = getNodeId(elemBelow); 
            
            // Logic to move placeholder
            if (targetObj && targetObj.element !== dragSrcEl && !dragSrcEl.contains(targetObj.element)) {
                const rect = targetObj.element.getBoundingClientRect();
                const relY = e.clientY - rect.top;
                const parent = targetObj.element.parentNode;
                
                // Insert before if in top half, after if in bottom half
                if (relY < rect.height / 2) {
                    parent.insertBefore(dropPlaceholder, targetObj.element);
                } else {
                    parent.insertBefore(dropPlaceholder, targetObj.element.nextSibling);
                }
            }
        }
    });
    ...
    document.addEventListener('mouseup', e => {
        ...
        if (isDragging) {
            ...
            if (dropPlaceholder && dropPlaceholder.parentNode) {
                const nextSib = dropPlaceholder.nextElementSibling;
                const prevSib = dropPlaceholder.previousElementSibling;
                const parent = dropPlaceholder.parentNode;
                
                const parentId = parent.dataset.vibeNodeId || (parent.id === 'whole-page' ? 'whole-page' : null);
                
                let targetId, position;
                
                // Prefer anchoring to siblings, else parent
                if (nextSib && nextSib.dataset.vibeNodeId && nextSib !== dragSrcEl) {
                    targetId = nextSib.dataset.vibeNodeId;
                    position = 'before';
                } else if (prevSib && prevSib.dataset.vibeNodeId && prevSib !== dragSrcEl) {
                    targetId = prevSib.dataset.vibeNodeId;
                    position = 'after';
                } else {
                    const parentObj = getNodeId(parent);
                    if (parentObj) {
                        targetId = parentObj.nodeId;
                        position = 'inside';
                    }
                }
                ...
                if (targetId && position) {
                    window.parent.postMessage({
                        type: 'vibe-node-move',
                        sourceNodeId: dragSrcEl.dataset.vibeNodeId,
        ...
        // Reset flag after a tick so click handler can read it
    ...
    Object.keys(oConsole).forEach(level => {
        window.console[level] = (...args) => {
            oConsole[level](...args); 
            try { window.parent.postMessage({ type: 'iframe-console', level, payload: args.map(p => serializeArg(p)) }, '*') } catch(e) { oConsole.error('Vibe proxy error', e) }
        }
    });
...
async function updateNodeByDescription(nodeId, newDescription, buttonEl = null) {
    ...
    try {
        if (node.type === 'container') {
            ...
            // Check if the AI wants to ask a question (unlikely for a simple update, but good safety)
            if (agentDecision.question) {
