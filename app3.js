...
**TASK:**
1.  **Design the Controls:** Decide what controls (sliders, color pickers) are needed.
2.  **Implement the Logic:** Determine what code changes (CSS or JavaScript) are required to make these controls functional. This might involve adding new CSS properties, creating new JavaScript variables, or writing new functions.
...
- The \`actions\` can modify ANY node in the tree, not just the one the controls are attached to. This is essential for separating concerns (e.g., updating a global CSS or JS file).`;
        ...
        // Part 3: Refresh UI and apply initial values
        refreshAllUI();
        ...
        updateDynamicStyles(); // This will apply both new and existing CSS controls
...
function updateDynamicJsVariable(control) {
    ...
    // ** THE FIX IS HERE **
    // Check if the value is not a number OR if its string representation is empty after trimming.
    // This safely handles both numeric values from the AI and string values from the input fields.
    if (isNaN(newValue) || String(newValue).trim() === '') {
    ...
    function traverseForControls(node) {
        if (node.controls && Array.isArray(node.controls)) {
            for (const control of node.controls) {
                if (control.controlTarget === 'js') continue; // Skip JS controls
