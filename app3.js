...
If the user says "control the particle count" and you find a node with id "effect-script" containing \`const PARTICLE_COUNT = 100;\`, your output should be:
{
  ...
  "targetJsVariable": "PARTICLE_COUNT"
        ...
        newControls.forEach(control => {
            handleAiControlUpdate(nodeId, control);
        });
...
function handleControlChange(event) {
    ...
    // On 'input', just update the visual label for sliders for real-time feedback.
    if (control.type === 'slider' && event.type === 'input') {
        const valueSpan = input.nextElementSibling;
        if (valueSpan && valueSpan.classList.contains('slider-value')) {
            valueSpan.textContent = `${newValue}${control.unit || ''}`;
        }
        return; // Don't trigger AI on every tiny movement.
    }

    // Only proceed with the full update logic on the 'change' event (e.g., slider released).
    if (event.type !== 'change') {
        return;
    }
    
    // Update the control's value in our tree state.
    if (String(control.value) !== newValue) {
        control.value = newValue;
    }

    // Trigger the new AI middleman to apply the change.
    handleAiControlUpdate(nodeId, control);
}

async function handleAiControlUpdate(nodeId, control) {
    const node = findNodeById(nodeId);
    if (!node) {
        console.error(`Node not found for AI control update: ${nodeId}`);
        return;
    }

    showGlobalAgentLoader(`AI applying change: ${control.label} to ${control.value}...`);

    try {
        const systemPrompt = `You are an expert AI developer. Your task is to intelligently modify the full code of a webpage based on a user changing a specific UI control that you previously generated.

**TASK:**
Analyze the user's action and update the provided 'Full Current Code' to reflect the change. Make the change in the most logical place. For example, if changing a color, it's better to update a CSS variable if one exists, rather than a single class.

**INPUT:**
1.  **Control Change:** The specific control that was changed and its new value.
2.  **Component Context:** The 'vibe node' that this control is attached to.
3.  **Full Current Code:** The entire HTML file that you need to modify.

**RULES:**
1.  Apply the new value from the control to the appropriate part of the code.
2.  **CRITICAL:** Your output must be ONLY the new, complete, and valid HTML code for the entire file. Do not provide explanations, diffs, snippets, or markdown formatting. Your entire response must be the raw HTML source code.`;
        ...
        const userPrompt = `The user changed a control. Please update the code accordingly.
...
**Component Context (the node this control belongs to):**
\`\`\`json
${JSON.stringify({ id: node.id, type: node.type, description: node.description }, null, 2)}
\`\`\`
...
`;
        ...
        // Use our existing robust function to parse the new code and refresh everything.
        await processCodeAndRefreshUI(newFullCode);

    } catch (error) {
        console.error("AI Control Update failed:", error);
        alert(`Failed to apply control change via AI: ${error.message}`);
    } finally {
        hideGlobalAgentLoader();
    }
}
