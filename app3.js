...
function jumpToNodeInCode(nodeId) {
    ...
    setTimeout(function() {
        ...
        var idx = findNodeCodeInFullCode(nodeId, code);
        ...
        var selEnd = Math.min(idx + nodeId.length + 20, code.length);
        fullCodeEditor.focus();
        fullCodeEditor.setSelectionRange(idx, selEnd);

        try {
            var linesBefore = code.substring(0, idx).split('\n');
            var lineNum = linesBefore.length;
            var totalLines = code.split('\n').length;
            var ratio = lineNum / totalLines;
            fullCodeEditor.scrollTop = ratio * fullCodeEditor.scrollHeight - fullCodeEditor.clientHeight / 2;
        } catch(e) {}
        ...
        console.log('Wiki: Jumped to node "' + nodeId + '" at char index ' + idx);
    }, 120);
}
