...
- The \`actions\` can modify ANY node in the tree, not just the one the controls are attached to. This is essential for separating concerns (e.g., updating a global CSS or JS file).`;
        ...
        // The UI refresh will automatically re-render the preview and execute all scripts with their new initial values.
...
async function handleControlChange(event) {
    ...
    if (control.controlTarget === 'js') {
        await updateJsVariableWithAI(control, input);
    } else {
        updateDynamicStyles();
    }
...
/**
 * Updates a JavaScript variable within a specific vibe node's code using a targeted AI call.
 * @param {object} control - The control object from the vibeTree.
 * @param {HTMLElement} inputElement - The UI input element that triggered the change.
 */
async function updateJsVariableWithAI(control, inputElement) {
    ...
    const controlWrapper = inputElement.closest('.control-wrapper');
    if (controlWrapper) controlWrapper.classList.add('loading');

    try {
        const systemPrompt = `You are a precise code modification assistant. Your task is to update a single variable's value within a provided JavaScript code block.
- You will receive the name of the variable to change, its new value, and the full code snippet.
- You MUST find the variable declaration (e.g., 'let myVar = 10;') and change ONLY its value.
- Preserve all other code, formatting, and comments.
- **OUTPUT ONLY the complete, updated code block.** Do not add explanations, markdown, or any other text.`;
        
        const userPrompt = `Update the variable named \`${control.targetJsVariable}\` to the new value \`${control.value}\`.
...
\`\`\``;
        ...
        const newCode = await callAI(systemPrompt, userPrompt, false);
        ...
        if (targetNode.code !== newCode) {
            console.log(`AI updated JS variable '${control.targetJsVariable}' in node '${targetNode.id}'.`);
            recordHistory(`Control change: ${control.targetJsVariable}`);
            targetNode.code = newCode;
            reExecuteAllScripts();
        }

    } catch (e) {
        console.error(`AI failed to update JS variable '${control.targetJsVariable}':`, e);
    } finally {
        if (controlWrapper) controlWrapper.classList.remove('loading');
    }
}
