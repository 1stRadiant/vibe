...
function addEventListeners() {
    ...
    document.querySelectorAll('.generate-controls-button').forEach(button => {
        button.addEventListener('click', handleGenerateControls);
    });
...
async function handleGenerateControls(event) {
    ...
    if (!node) {
        console.error("Node not found for control generation:", nodeId);
        return;
    }
    ...
    try {
        const systemPrompt = `You are an expert UI/UX designer. Your task is to analyze a Vibe Tree node and generate a JSON array of useful control objects for editing its appearance and layout.

**OUTPUT:** You MUST respond with a single, valid JSON array of control objects. Do not include any explanatory text or markdown.
...
**ANALYSIS:**
1. Analyze the node's HTML code to find its main element ID. Use this for the 'selector'.
2. Based on the node's type, code, and description, suggest relevant controls. Examples:
   - For layout containers: suggest controls for 'gap', 'padding', 'justifyContent'.
   - For text elements: suggest 'color', 'fontSize', 'fontWeight'.
   - For elements with a background: suggest 'backgroundColor', 'borderRadius'.
3. Infer reasonable initial values, min/max ranges, and units from the context.`;

        const userPrompt = `Generate controls for the following node:\n\n\`\`\`json\n${JSON.stringify(node, null, 2)}\n\`\`\`\n\nFull Project Tree for Context:\n\`\`\`json\n${JSON.stringify(vibeTree, null, 2)}\n\`\`\``;
        ...
        console.log(`Generated ${newControls.length} controls for node ${nodeId}`);
        
        refreshAllUI();
        updateDynamicStyles(); // Apply initial values
        autoSaveProject();

    } catch (e) {
        console.error(`Failed to generate controls for ${nodeId}:`, e);
        alert(`AI failed to generate controls: ${e.message}`);
    } finally {
        button.disabled = false;
        button.innerHTML = originalHtml;
    }
...
function bindEventListeners() {
    ...
    // START OF CHANGE: Added listeners for new save buttons
