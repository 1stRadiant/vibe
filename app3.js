...
// START OF CHANGE: Add new modal elements for AI Project Edit
...
// --- Application State ---
// START OF CHANGE: Add user state management and agent task queue
...
// START OF CHANGE: Add a function to get database instructions for the AI
/**
...
function addEventListeners() {
    document.querySelectorAll('.update-button').forEach(button => {
     ...
     // START OF CHANGE: Add listener for the new "AI Edit Project" button
...
// --- NEW: Iterative Agent Task Queue Logic ---

/**
 * Renders the current task queue in the UI, including remove buttons.
 * This is called whenever the taskQueue array changes.
 */
function renderTaskQueue() {
    ...
    taskQueue.forEach((task, index) => {
        ...
        // Sanitize task display to prevent HTML injection
...
async function processNextTaskInQueue() {
    ...
    const progressText = `Processing task ${taskQueue.length - taskQueue.length + 1} of ${taskQueue.length}`;
    showGlobalAgentLoader('Agent is working...', progressText);
    logToAgent(`---<br><strong>Starting Task (${taskQueue.length} remaining):</strong> ${currentTask}`, 'plan');

    try {
        await runIterativeSessionForTask(currentTask);
...
/**
 * Fully automates the planning and execution of a single task, returning a promise
 * that resolves on completion or rejects on failure.
 * @param {string} taskPrompt The natural language goal for the task.
 */
function runIterativeSessionForTask(taskPrompt) {
    return new Promise(async (resolve, reject) => {
        // 1. Planning Phase
        iterativeSessionState = {
            status: 'planning',
            overallGoal: taskPrompt,
            plan: [],
            currentStepIndex: 0,
            history: [{ role: 'user', content: `Task Goal: ${taskPrompt}` }]
        };
        updateIterativeUI();
        logToAgent(`Generating a plan for: "${taskPrompt}"...`, 'info');
        showAgentSpinner();
        
        try {
            const systemPrompt = getIterativePlannerSystemPrompt();
            const userPrompt = `My website goal for this task is: "${taskPrompt}"`;
            const rawResponse = await callAI(systemPrompt, userPrompt, true);
            const responseJson = JSON.parse(rawResponse);
            if (!responseJson.plan || !Array.isArray(responseJson.plan)) {
                throw new Error("AI did not return a valid plan array for the task.");
            }
            iterativeSessionState.plan = responseJson.plan;
            iterativeSessionState.status = 'executing';
        } catch (error) {
            hideAgentSpinner();
            return reject(error); // Planning failed
        }
        
        logToAgent(`<strong>Plan Generated.</strong> Starting execution of ${iterativeSessionState.plan.length} steps.`, 'plan');
        updateIterativeUI();

        // 2. Execution Phase (Loop through all steps)
        for (let i = 0; i < iterativeSessionState.plan.length; i++) {
            if (!isTaskQueueRunning) return reject(new Error("Queue was stopped by the user."));

            iterativeSessionState.currentStepIndex = i;
            updateIterativeUI();
            const currentStepDescription = iterativeSessionState.plan[i];
            
            try {
                logToAgent(`<strong>Executing Step ${i + 1}/${iterativeSessionState.plan.length}:</strong> ${currentStepDescription}`, 'action');
                const systemPrompt = getIterativeExecutorSystemPrompt();
                const userPrompt = `Overall Task Goal: "${taskPrompt}"\nFull Plan:\n${iterativeSessionState.plan.map((s, idx) => `${idx+1}. ${s}`).join('\n')}\nCurrent Step to Execute: "${currentStepDescription}"\nCurrent Vibe Tree:\n\`\`\`json\n${JSON.stringify(vibeTree, null, 2)}\n\`\`\``;
                
                const rawResponse = await callAI(systemPrompt, userPrompt, true);
                const agentDecision = JSON.parse(rawResponse);
                executeAgentPlan(agentDecision, logToAgent);
                
                await testCurrentStep(); // Check for runtime errors
                logToAgent(`Step ${i + 1} completed successfully.`, 'info');
                await new Promise(res => setTimeout(res, 1000));
            } catch (error) {
                hideAgentSpinner();
                iterativeSessionState.status = 'paused';
                updateIterativeUI();
                return reject(error); // Step failed
            }
        }
...
/**
 * Toggles the visibility of UI elements based on the agent's state.
 */
function updateIterativeUI() {
    // Part 1: Manage the main agent control buttons and prompt
    if (runAgentSingleTaskButton) {
        runAgentSingleTaskButton.textContent = 'Add Task to Queue';
        runAgentSingleTaskButton.disabled = isTaskQueueRunning;
    }
    if (startIterativeSessionButton) {
        startIterativeSessionButton.textContent = `Start Queue (${taskQueue.length})`;
    ...
    // Part 2: Manage the UI for the CURRENTLY executing task
    const status = iterativeSessionState.status;
    const isTaskActive = status !== 'idle' && isTaskQueueRunning;
    
    iterativeSessionUI.classList.toggle('hidden', !isTaskActive);
    
    if (isTaskActive) {
        iterativeControls.classList.remove('hidden');
        acceptContinueButton.style.display = 'none'; // Not used in automated queue
        requestChangesButton.style.display = 'none'; // Not used in automated queue
        endSessionButton.style.display = 'inline-block';
        endSessionButton.textContent = 'Stop Queue';
        
        iterativePlanDisplay.innerHTML = '';
        const header = document.createElement('h4');
        header.textContent = `Current Task: ${iterativeSessionState.overallGoal}`;
        iterativePlanDisplay.appendChild(header);
...
/**
 * Resets the state and UI, ending the iterative session and stopping the queue.
 */
function handleEndIterativeSession() {
    if (isTaskQueueRunning) {
        logToAgent('Task queue stopped by user.', 'info');
        isTaskQueueRunning = false; // This will cause the running promise to reject.
    }
    
    iterativeSessionState = {
        status: 'idle', overallGoal: '', plan: [], currentStepIndex: -1, history: []
    };
    
    updateIterativeUI();
    renderTaskQueue(); // Re-enable remove buttons if any tasks remain
    hideAgentSpinner();
    hideGlobalAgentLoader();
}
...
// START OF CHANGE: Logic for the new AI Project Edit modal
/**
...
function switchToTab(tabId) {
    ...
    // START OF CHANGE: Inject task queue container when agent tab is opened
    if (tabId === 'agent') {
        ...
        updateIterativeUI(); // Ensure UI is correct on tab switch
    }
    // END OF CHANGE
}
...
function bindEventListeners() {
    ...
    // START OF CHANGE: Update agent listeners to use the new task queue system
    ...
    // These two buttons are no longer used for their original purpose but their listeners are harmless.
    if (acceptContinueButton) acceptContinueButton.addEventListener('click', () => {});
    if (requestChangesButton) requestChangesButton.addEventListener('click', () => {});
    ...
    // START OF CHANGE: Add listeners for the new AI Project Edit modal
    ...
    window.addEventListener('click', (event) => {
        ...
        if (event.target === aiProjectEditModal) closeAiProjectEditModal(); // Close on backdrop click
    });
