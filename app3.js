...
function handleDragStart(event) {
    // Ensure drag only starts on the handle itself.
    ...
    const targetNode = handle.closest('.vibe-node');
    if (!targetNode) {
        return;
    }
    ...
    // Use a timeout to allow the browser to create the drag image before we modify the element's class
    setTimeout(() => {
...
function handleDragOver(event) {
    ...
    // If there's no valid target or we're over the item being dragged, clear indicators and exit
    if (!targetNode || targetNode.dataset.nodeId === draggedNodeId) {
        if (lastDragOverEl) {
            lastDragOverEl.classList.remove('drop-indicator-before', 'drop-indicator-after');
            lastDragOverEl = null;
        }
        return;
    }

    // If we've moved to a new element, clear the old one's indicators
    if (lastDragOverEl && lastDragOverEl !== targetNode) {
        lastDragOverEl.classList.remove('drop-indicator-before', 'drop-indicator-after');
    }
    
    // Update the current element we're hovering over
    lastDragOverEl = targetNode;

    const rect = targetNode.getBoundingClientRect();
    const isAfter = event.clientY > rect.top + rect.height / 2;
    
    // Set the appropriate indicator on the current element
    if (isAfter) {
        targetNode.classList.remove('drop-indicator-before');
        targetNode.classList.add('drop-indicator-after');
    } else {
        targetNode.classList.remove('drop-indicator-after');
        targetNode.classList.add('drop-indicator-before');
    }
}

function handleDragLeave(event) {
    // This event can be noisy. Our handleDragOver logic is robust enough to handle most cases.
    // We only need to clean up if the cursor leaves the entire editor area.
...
function handleDragEnd() {
    // This is the final cleanup, running whether the drop was successful or cancelled.
    ...
    // Ensure all drop indicators are removed
    if (lastDragOverEl) {
        lastDragOverEl.classList.remove('drop-indicator-before', 'drop-indicator-after');
        lastDragOverEl = null;
    }
