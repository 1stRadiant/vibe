...
function updateDynamicJsVariable(control) {
    ...
    // A regex to find a variable declaration (const, let, var) and update its value.
    // This handles numbers, and strings in single or double quotes.
    const regex = new RegExp(`(const|let|var)\\s+(${variableName})\\s*=\\s*(['"]?)(.*?)\\3;`);
    
    if (!targetNode.code.match(regex)) {
        console.error(`Could not find variable "${variableName}" in node "${targetNode.id}". Regex failed.`);
        return;
    }
    ...
    if (isNaN(newValue) || newValue.trim() === '') {
        // It's a string, wrap it in quotes. Let's default to double quotes.
        replacementValue = `"${newValue.replace(/"/g, '\\"')}"`;
    } else {
        // It's a number.
        replacementValue = newValue;
    }

    const newCode = targetNode.code.replace(regex, `$1 $2 = ${replacementValue};`);
