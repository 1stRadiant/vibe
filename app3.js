...
async function handleShareProject() {
    ...
    // START OF CHANGE: Disallow sharing of raw HTML projects via sharable link
...
function renderEditor(node) {
    // START OF CHANGE: Handle rendering for raw HTML projects
    if (node && node.type === 'raw-html-container') {
        ...
        placeholderEl.innerHTML = `
            ...
            <p>To enable the editor, use the "Update from Code" button in the "Full Code" tab. This will use AI to structure your project.</p>
        `;
...
async function handleFileUpload() {
    ...
    reader.onload = async (event) => {
        ...
        currentProjectStorageType = 'local'; // Default uploads to local
        ...
        // Set up application state for the new project
        updateSaveButtonStates();
        if(openAiStructureModalButton) openAiStructureModalButton.disabled = true;
        ...
        // Create the special raw HTML vibeTree
        vibeTree = {
        ...
        resetHistory();
        refreshAllUI(); // This will trigger the modified render/apply functions
        autoSaveProject(); // Save the new raw project
        switchToTab('preview');
...
function assembleMultiFileBundle(tree = vibeTree) {
    // If it's a raw project, the bundle is simpler.
...
function applyVibes() {
    try {
        ...
        // START OF CHANGE: Handle raw HTML projects for instant preview
        if (vibeTree && vibeTree.type === 'raw-html-container') {
            html = vibeTree.code || '';
        } else {
