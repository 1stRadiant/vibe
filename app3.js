...
const saveToGithubButton = document.getElementById('save-to-github-button'); // GITHUB
...
let currentAgentTaskContext = ""; // Stores conversation for current task
...
function onLoginSuccess(userData) {
    ...
    populateProjectList();
    resetToStartPage();
    updateSaveButtonStates(); // Ensures cloud save is enabled if a project is already loaded
}
...
const githubApi = {
    ...
    getProjectFile: async (owner, repo, branch) => {
        try {
            ...
            if (error.message.includes("404")) return null; // File doesn't exist
            throw error;
        }
    },
...
async function populateGithubRepos() {
    try {
        ...
        repos
            .filter(repo => repo.permissions.push) // Only show repos user can write to
            .forEach(repo => {
...
async function handleLoadFromGithub() {
    ...
    try {
        ...
        if (fileContent === null) {
            ...
            currentProjectId = repo; // Use repo name as project ID
        }
...
/**
 * Prompts the user for a new project ID and checks for conflicts in the target storage.
 * @param {'cloud' | 'local'} storageType - The type of storage to check against.
 * @param {string} suggestedId - The current project ID to suggest as a default.
 * @returns {Promise<string|null>} The new, validated project ID, or null if the user cancels.
 */
async function getNewProjectId(storageType, suggestedId) {
    ...
    if (!newId) {
        ...
        return getNewProjectId(storageType, suggestedId); // Recurse
    }
    ...
    if (existingProjects.includes(newId)) {
        if (!confirm(`A ${storageType} project named "${newId}" already exists. Do you want to overwrite it?`)) {
            return getNewProjectId(storageType, newId); // Recurse
        }
    }
...
/**
 * Updates the disabled state of the save buttons based on the current app state.
 */
...
/**
 * Handles the "Save to Cloud" action.
 */
async function handleSaveToCloud() {
    ...
    try {
        ...
        currentProjectGithubSource = null; // No longer a GitHub project
...
/**
 * Handles the "Save to Local" action.
 */
...
function updateFeatureAvailability() {
    ...
    // uploadHtmlButton is not AI-powered, should remain enabled.
...
/**
 * Generates instructions for the AI on how to generate images using Pollinations.
 * @returns {string} The formatted instructions.
 */
...
**IMPLEMENTATION:**
- Insert this URL directly into HTML \`<img>\` tags: \`<img src="https://pollinations.ai/p/..." ...>\`
- Insert this URL directly into CSS: \`background-image: url('https://pollinations.ai/p/...');\`
- Do NOT use generic placeholders like 'via.placeholder.com' or '#'. Always generate a custom Pollinations URL.
---
`;
}
...
async function callAI(systemPrompt, userPrompt, forJson = false, streamCallback = null) {
    ...
    const imageContext = getImageGenerationInstructions();
    const augmentedSystemPrompt = systemPrompt + componentContext + imageContext;
...
function parseHtmlToVibeTree(fullCode) {
    ...
    // REFACTOR: Use a robust method that doesn't split code by regex, avoiding broken blocks.
    const scriptTags = Array.from(doc.querySelectorAll('script'));
    scriptTags.forEach((scriptTag, index) => {
        // Skip external scripts for now, as we can't easily inline them without fetching
...
async function handleUpdateTreeFromCode() {
    ...
    // If the project is currently in raw-html mode, updating the code will also update the preview
    // without converting to a vibe tree. The user must click the "Process" button for that.
    if (vibeTree && vibeTree.type === 'raw-html-container') {
        vibeTree.code = fullCode;
        recordHistory("Update raw HTML code");
        applyVibes(); // Refresh preview instantly
        autoSaveProject();
    ...
    // If it's a normal project, or if the user clicks the "Process" button, we run the full decomposition.
...
`;
        ...
        // If it was a raw project, just update the code and refresh the preview.
        if (vibeTree && vibeTree.type === 'raw-html-container') {
             vibeTree.code = newFullCode;
             recordHistory("AI update to raw HTML");
             refreshAllUI();
        } else {
            // Otherwise, run the full decomposition process.
...
3.  **Full Generated Code:** The final, rendered HTML, CSS, and JS code of the current website. Use this to understand the context of the Vibe Tree components.

**INTERACTIVE CLARIFICATION:**
If the user's request is ambiguous, lacks necessary detail (e.g., 'make it look better' without specifying how), or if you strongly recommend adding a feature the user didn't ask for (e.g., 'adding a backend connection' when they just asked for a form UI), you MUST ask for confirmation first.
In this case, return a JSON object with ONLY a 'question' field:
{ "question": "Your question or proposal here." }
Do NOT return a plan or actions if you are asking a question.
...
**OUTPUT:**
You must respond ONLY with a single, valid JSON object. Do not add any other text or markdown.
...
function handleAddTaskToQueue() {
    ...
    // If waiting for confirmation, treat this input as a reply instead of a new task
    if (waitingForAgentConfirmation) {
        ...
        updateTaskQueueUI(); // Refresh UI to disabled state
        
        // Re-run the current task execution with updated context
        executeSingleTask(currentAgentTaskContext)
            ...
            .catch(error => {
                // If the error is simply because we are waiting again (another question), do nothing.
...
async function processNextTaskInQueue() {
    ...
    // Initialize context if starting fresh on a task
    ...
    try {
        ...
        // If we are waiting for confirmation, stop the loop here.
        if (waitingForAgentConfirmation) {
            updateTaskQueueUI(); // Enable input for reply
            return; 
        }

        logToAgent(`<strong>Task completed successfully:</strong> ${currentTask}`, 'info');
        taskQueue.shift(); // Remove completed task
        currentAgentTaskContext = ""; // Reset context
        renderTaskQueue();
        updateTaskQueueUI();

        setTimeout(processNextTaskInQueue, 1500);
    } catch (error) {
        // If waiting for confirmation, treat as non-fatal pause
        if (waitingForAgentConfirmation) {
            updateTaskQueueUI();
            return;
        }
 ...
 * @param {string} promptContext The full conversation context for the current task.
 */
async function executeSingleTask(promptContext) {
    ...
    try {
        ...
        if (agentDecision.question) {
            logToAgent(`<strong>AI Question:</strong> ${agentDecision.question}`, 'plan');
            currentAgentTaskContext += `\nAI: ${agentDecision.question}`;
            waitingForAgentConfirmation = true;
            hideGlobalAgentLoader(); // Hide full loader to allow user interaction
            return; // Exit without executing plan
        }

        executeAgentPlan(agentDecision, logToAgent);
        waitingForAgentConfirmation = false; // Plan executed, no longer waiting
    } catch(e) {
        throw e;
    } finally {
        hideAgentSpinner();
...
function updateTaskQueueUI() {
    if (runAgentSingleTaskButton) {
        if (waitingForAgentConfirmation) {
            ...
            runAgentSingleTaskButton.classList.add('pulse-button'); // Optional visual cue
        } else {
...
/**
 * Opens the AI Project Edit modal, pre-filled with the context of the selected node.
 * @param {Event} event The click event from the "AI Edit Project" button.
 */
...
/**
 * Closes the AI Project Edit modal.
 */
...
/**
 * Executes the global AI edit based on the focus node and user prompt.
 */
...
// START OF CHANGE: Add logic for the new AI Structure Update modal
...
function autoSaveProject() {
    ...
    switch(currentProjectStorageType) {
        ...
        case 'local':
            ...
            break;
        case 'github':
            // GitHub saves are manual via the save button, so we don't auto-save.
            break;
    }
}
...
/**
 * Updates a JavaScript variable within a specific vibe node's code.
 * @param {object} control - The control object from the vibeTree.
 */
...
// END: Dynamic Controls Feature

// --- START OF AUTOMATION SYSTEM ---

// 1. Inject CSS for Automation UI
const automationStyles = document.createElement('style');
automationStyles.textContent = `
    #automation-modal { display: none; position: fixed; z-index: 10001; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.5); backdrop-filter: blur(5px); }
    .automation-highlight { outline: 3px solid #ff00ff !important; box-shadow: 0 0 15px #ff00ff !important; transition: all 0.2s; z-index: 9999; }
    #automation-log { background: #1e1e1e; color: #0f0; font-family: monospace; padding: 10px; height: 150px; overflow-y: auto; margin-top: 10px; border-radius: 4px; font-size: 12px; }
    .automation-step-active { background-color: rgba(255, 0, 255, 0.2); border-left: 3px solid #ff00ff; padding-left: 5px; }
`;
document.head.appendChild(automationStyles);

// 2. Create the Automation Button and Modal programmatically
function createAutomationUI() {
    // Add button to top bar (inserting before Settings or similar)
    const topBar = document.querySelector('.top-bar-right') || document.body;
    // Check if button already exists to prevent duplicates on reload
    if (document.getElementById('open-automation-button')) return;

    const autoBtn = document.createElement('button');
    autoBtn.id = 'open-automation-button';
    autoBtn.className = 'action-button';
    autoBtn.innerHTML = '🤖 Auto';
    autoBtn.title = "Open UI Automation Agent";
    autoBtn.style.marginRight = "10px";
    
    // Insert before the generic user dropdown or settings
    if(topBar.firstChild) {
        topBar.insertBefore(autoBtn, topBar.firstChild);
    } else {
    ...
    <div id="automation-modal" class="modal">
        <div class="modal-content" style="max-width: 600px;">
            <div class="modal-header">
                <h3>UI Automation Agent</h3>
                <span class="close-button" id="close-automation-modal">&times;</span>
            </div>
            <div class="modal-body">
                <p>Describe a sequence of actions you want to perform.</p>
                <textarea id="automation-prompt" rows="3" placeholder="e.g., Switch to the Agent tab, type 'Fix the header' and run the task." style="width:100%; margin-bottom:10px;"></textarea>
                
                <div style="display:flex; gap:10px; margin-bottom:15px;">
                    <button id="generate-script-button" class="action-button ai-powered-button">✨ Generate Script</button>
                    <button id="run-script-button" class="action-button" disabled>▶ Run Script</button>
                    <button id="stop-script-button" class="action-button" style="background:#d9534f; display:none;">⏹ Stop</button>
                </div>

                <details>
                    <summary>View/Edit Generated JSON Script</summary>
                    <textarea id="automation-json-script" rows="10" style="width:100%; font-family:monospace; background:#282c34; color:#abb2bf; margin-top:5px;"></textarea>
                </details>

                <div id="automation-log">Waiting for input...</div>
            </div>
        </div>
    </div>`;
...
// 3. UI Element Map - Knowledge Base for the AI
// This maps human concepts to specific DOM selectors in your app.
const UI_MAP = {
    // Tabs
    ...
    // Preview Controls
    "Inspect Button": "#toggle-inspect-button",
    "Undo Button": "#undo-button",
    "Redo Button": "#redo-button",
    
    // Project Actions
    "Share Button": "#share-project-button",
    "Save Cloud Button": "#save-to-cloud-button",
    "Save Local Button": "#save-to-local-button",
    "New Project Button": "#new-project-button",

    // Code Editor
    "Update Tree From Code": "#update-tree-from-code-button",
    "Full Code Editor": "#full-code-editor",
    
    // Agent
    "Agent Input": "#agent-prompt-input",
    "Run Agent Task": "#run-agent-single-task-button",
    "Start Iterative Session": "#start-iterative-session-button",
    ...
    "Gemini Model Select": "#gemini-model-select",
    
    // Modals (Generic access)
    "Close Modal Button": ".close-button",
    "Confirm Button": ".confirm-button, button:contains('Yes')", // Conceptual selector
};
...
const AutomationEngine = {
    ...
    log: (msg, type = 'info') => {
        ...
        if (type === 'success') line.style.color = '#98c379';
        logEl.appendChild(line);
        logEl.scrollTop = logEl.scrollHeight;
    },

    // Highlight element visually before interacting
    ...
    execute: async (scriptJson) => {
        const script = typeof scriptJson === 'string' ? JSON.parse(scriptJson) : scriptJson;
        AutomationEngine.isRunning = true;
        document.getElementById('stop-script-button').style.display = 'inline-block';
        document.getElementById('run-script-button').disabled = true;
        document.getElementById('automation-modal').style.display = 'none'; // Hide modal to see action

        try {
            for (let i = 0; i < script.length; i++) {
                if (!AutomationEngine.isRunning) break;
                
                const step = script[i];
                console.log(`Executing step: ${step.action}`, step);
                ...
                // --- ACTION: LOG ---
                if (step.action === 'log') {
                    // Just log to console/toast, no UI interaction
                    console.info("Auto:", step.message);
                    continue;
                }
                ...
                // Resolve selector (handle named lookups from UI_MAP or raw CSS selectors)
                const selector = UI_MAP[step.target] || step.target;
                
                // If it's a tab switch, we can use the global helper or click the element
                if (step.action === 'switchTab') {
                    ...
                    // Fallback to clicking selector if specific function fails
                    target = document.querySelector(`.tab-button[data-tab="${tabId}"]`);
                } else {
                ...
                // Perform Action
                await AutomationEngine.highlight(target);

                if (step.action === 'click') {
                    target.click();
                    // If clicking an input/textarea, focus it
            ...
            AutomationEngine.log("Script completed successfully.", "success");

        } catch (e) {
            document.getElementById('automation-modal').style.display = 'block';
            AutomationEngine.log(`Error: ${e.message}`, "error");
            console.error(e);
        } finally {
            AutomationEngine.isRunning = false;
            document.getElementById('stop-script-button').style.display = 'none';
            document.getElementById('run-script-button').disabled = false;
        }
    }
};
...
async function generateAutomationScript() {
    ...
    const btn = document.getElementById('generate-script-button');
    btn.disabled = true;
    btn.innerHTML = 'Thinking...';
    AutomationEngine.log("Generating script from AI...");

    // Construct knowledge base string
    const mapString = Object.entries(UI_MAP).map(([name, sel]) => `- "${name}": ${sel}`).join('\n');

    const systemPrompt = `You are a browser automation expert. You generate JSON execution scripts to control a web application based on user prompts.

**AVAILABLE UI ELEMENTS (Name: Selector):**
${mapString}

**COMMANDS:**
1. { "action": "click", "target": "Name or Selector" } -> Clicks an element.
2. { "action": "type", "target": "Name or Selector", "value": "text to type" } -> Types into an input.
3. { "action": "wait", "ms": 1000 } -> Pauses execution (use before interacting with things that load).
4. { "action": "switchTab", "tabId": "preview|code|agent|chat|files" } -> Special command to switch tabs. Use this instead of clicking tab buttons if possible.

**RULES:**
- Return ONLY a valid JSON array. No markdown, no comments.
- If the user asks to "navigate" to a section, use "switchTab".
- Use the provided "Names" from the UI Elements list for the "target" field whenever possible.
- Include "wait" commands reasonably (e.g., after switching tabs or submitting forms).

**EXAMPLE:**
User: "Go to agent, type 'Hello', and run."
Output:
[
  { "action": "switchTab", "tabId": "agent" },
  { "action": "wait", "ms": 500 },
  { "action": "type", "target": "Agent Input", "value": "Hello" },
  { "action": "click", "target": "Run Agent Task" }
]`;

    const userPrompt = `Generate a script for: "${prompt}"`;

    try {
        // Reuse your existing callAI function
        const response = await callAI(systemPrompt, userPrompt, true);
        
        // Sanitize response to ensure just JSON
        let jsonStr = response;
        const match = response.match(/\[.*\]/s); // Find array
        if (match) jsonStr = match[0];

        // Validate JSON
        JSON.parse(jsonStr);

        document.getElementById('automation-json-script').value = jsonStr;
        document.getElementById('run-script-button').disabled = false;
        AutomationEngine.log("Script generated. Review JSON and click Run.","success");

    } catch (e) {
        AutomationEngine.log(`AI Error: ${e.message}`, "error");
    } finally {
        btn.disabled = false;
...
// 6. Event Listeners for Automation
document.addEventListener('DOMContentLoaded', () => {
    // Ensure UI is created if not already present
    createAutomationUI();

    const openBtn = document.getElementById('open-automation-button');
    const closeBtn = document.getElementById('close-automation-modal');
    const modal = document.getElementById('automation-modal');
    const genBtn = document.getElementById('generate-script-button');
    const runBtn = document.getElementById('run-script-button');
    const stopBtn = document.getElementById('stop-script-button');
    ...
    if(genBtn) genBtn.onclick = generateAutomationScript;
    
    if(runBtn) runBtn.onclick = () => {
        const script = document.getElementById('automation-json-script').value;
        if(script) AutomationEngine.execute(script);
    };
    
    if(stopBtn) stopBtn.onclick = () => {
        AutomationEngine.isRunning = false;
        AutomationEngine.log("Script stopped by user.", "error");
    };
