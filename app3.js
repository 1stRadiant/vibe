...
function bindEventListeners() {
    ...
    // Add logic to inject the Merger Button if it doesn't exist
    const tabButtonsContainer = document.querySelector('.tab-buttons');
    if (tabButtonsContainer && !document.querySelector('[data-tab="merger"]')) {
        const mergerBtn = document.createElement('button');
        mergerBtn.className = 'tab-button';
        mergerBtn.dataset.tab = 'merger';
        mergerBtn.innerHTML = '<i class="bi bi-intersect"></i> Patch Tool';
        tabButtonsContainer.appendChild(mergerBtn);
    }
}
...
/* ========================================================
   PATCH TOOL INTEGRATION (Merger System)
   ======================================================== */

// 1. Inject Styles for the Patch Tool
function injectMergerStyles() {
    if (document.getElementById('merger-styles')) return;
    const style = document.createElement('style');
    style.id = 'merger-styles';
    style.textContent = `
        /* --- DIFF & PATCH TOOL STYLES --- */
        .merger-container { display: flex; flex-direction: column; gap: 15px; height: 100%; padding: 10px; box-sizing: border-box; }
        .merger-controls { display: flex; flex-wrap: wrap; gap: 15px; padding: 15px; background: #252526; border: 1px solid #3c3c3c; border-radius: 6px; align-items: center; }
        .merger-workspace { display: flex; gap: 20px; flex: 1; min-height: 0; }
        .merger-column { flex: 1; display: flex; flex-direction: column; gap: 10px; min-width: 0; }
        .merger-textarea { width: 100%; height: 100%; background: #1e1e1e; color: #d4d4d4; border: 1px solid #3c3c3c; padding: 15px; font-family: 'Consolas', monospace; font-size: 13px; line-height: 1.5; resize: none; border-radius: 4px; box-sizing: border-box; white-space: pre; }
        
        .diff-list { overflow-y: auto; display: flex; flex-direction: column; gap: 20px; padding-right: 5px; height: 100%; padding-bottom: 40px; }
        .diff-card { background: #252526; border: 1px solid #3c3c3c; border-radius: 6px; display: flex; flex-direction: column; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }
        .diff-header { padding: 10px 15px; display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 1px solid #3c3c3c; background: #2d2d2d; border-radius: 6px 6px 0 0; flex-wrap: wrap; gap: 10px; }
        ...
        .diff-btn-row { display: flex; gap: 8px; }
        
        .code-compare-container { display: grid; grid-template-columns: 1fr 1fr; background: #1e1e1e; font-family: 'Consolas', monospace; font-size: 12px; line-height: 1.5; border-radius: 0 0 6px 6px; overflow: hidden; }
        .code-compare-container.insert-mode { grid-template-columns: 1fr; }
        .code-pane { display: flex; overflow-x: auto; padding: 10px 0; position: relative; }
        .original { background-color: #2d1818; border-right: 1px solid #3c3c3c; }
        .replacement { background-color: #122615; }
        .insert-only { background-color: #122615; }
        .line-numbers { text-align: right; padding: 0 10px; color: #858585; border-right: 1px solid #3c3c3c; user-select: none; min-width: 30px; }
        .code-content { padding: 0 10px; white-space: pre; color: #d4d4d4; tab-size: 4; flex: 1; outline: none; }
        .code-content[contenteditable="true"]:focus { background-color: #18301b; }
        
        .status-badge { display: inline-block; padding: 2px 8px; border-radius: 12px; font-size: 0.75em; text-transform: uppercase; font-weight: bold; margin-top: 5px; align-self: flex-start; }
        .status-pending { background: #444; color: #ddd; }
        .status-approved { background: #2ea043; color: white; }
        .status-rejected { background: #3c3c3c; color: #888; text-decoration: line-through; }
        
        @media (max-width: 900px) {
            .merger-workspace { flex-direction: column; }
            .merger-column { min-height: 400px; }
...
// 2. Logic Namespace for Merger Tool
const MergerTool = {
    sourceCode: "",
    rawOriginalSource: "",
    sourceFilename: "merged-file.js",
    proposedPatches: [],
    globalSourceSegments: [],

    init() {
        injectMergerStyles();
    },

    loadSource(content, filename) {
        this.rawOriginalSource = content;
        this.sourceCode = content;
        this.sourceFilename = filename || "merged-file.js";
        this.checkReady();
    },

    checkReady() {
        const analyzeBtn = document.getElementById('mergerAnalyzeBtn');
        const aiInput = document.getElementById('mergerAiInput');
        if (analyzeBtn && this.rawOriginalSource && aiInput.value.trim().length > 0) {
    ...
    parseCodeSegments(code) {
            ...
            if (!inBlock) {
                ...
                if (open > close) {
                    ...
                    segments.push({
                        ...
                        endIndex: charCount + lineLength - 1
                    });
                    currentContent = [];
                }
            } else {
                ...
                if (braceDepth <= 0) {
                    ...
                    const name = this.extractName(currentSig);
                    segments.push({
                        name: name,
                        signature: currentSig.trim(),
                        content: contentStr,
                        startLine: startLineNum + 1,
                        startIndex: startIndex,
                        endIndex: charCount + lineLength - 1
        ...
        if (currentContent.length > 0) {
             const contentStr = currentContent.join('\n');
             segments.push({ name: null, signature: currentSig.trim(), content: contentStr, startLine: startLineNum + 1, startIndex: startIndex, endIndex: charCount });
        }
        return segments;
    },

    extractName(str) {
        let m = str.match(/(?:function|class|const|let|var)\s+([a-zA-Z0-9_$]+)/);
        if(m) return m[1];
        return null;
    },

    cleanContent(str) { return str.replace(/\s+/g, '').trim(); },
    ...
    performAnalysis(updateCode) {
        this.globalSourceSegments = this.parseCodeSegments(this.rawOriginalSource);
        const updateSegments = this.parseCodeSegments(updateCode);
        this.proposedPatches = [];
        
        const diffContainer = document.getElementById('mergerDiffContainer');
        ...
        updateSegments.forEach((uSeg) => {
            ...
            if (uSeg.name) {
                const match = this.globalSourceSegments.find((sSeg, idx) => 
                    !matchedSourceIndices.has(idx) && sSeg.name === uSeg.name
                );
                if (match) bestMatch = { seg: match, reason: `Matched Name: "${uSeg.name}"`, index: this.globalSourceSegments.indexOf(match) };
            }
            // 2. Similarity Match
            if (!bestMatch) {
                const match = this.globalSourceSegments.find((sSeg, idx) => {
                    if (matchedSourceIndices.has(idx)) return false;
                    return this.similarity(uSeg.signature, sSeg.signature) > 0.85;
                });
                if (match) bestMatch = { seg: match, reason: "Matched Similar Signature", index: this.globalSourceSegments.indexOf(match) };
            }

            if (bestMatch) {
                matchedSourceIndices.add(bestMatch.index);
                if (this.cleanContent(bestMatch.seg.content) !== this.cleanContent(uSeg.content)) {
                    this.addPatchUI('replace', bestMatch.seg, uSeg, bestMatch.reason);
                }
            } else {
                this.addPatchUI('insert', null, uSeg, "No matching block found");
            }
        });

        const dlBtn = document.getElementById('mergerDownloadBtn');
        if (this.proposedPatches.length === 0) {
    ...
    addPatchUI(type, origSeg, newSeg, reason) {
        const id = Date.now() + Math.random();
        const patch = { id, type, original: origSeg, new: newSeg, status: 'pending' };
        this.proposedPatches.push(patch);
            ...
            bodyHtml = `
                <div class="code-compare-container">
                    <div class="code-pane original">
                        <div class="line-numbers">${this.genLineNums(origSeg.content, origSeg.startLine)}</div>
                        <div class="code-content">${this.escapeHtml(origSeg.content)}</div>
                    </div>
                    <div class="code-pane replacement">
                        <div class="line-numbers" id="ln-${id}">${this.genLineNums(newSeg.content, newSeg.startLine || 1)}</div>
                        <div class="code-content" id="edit-${id}" contenteditable="true" spellcheck="false">${this.escapeHtml(newSeg.content)}</div>
                    </div>
                </div>`;
            headerRightHtml = `
                <div class="diff-btn-row">
                    <button class="action-button small" style="background:#2ea043" onclick="MergerTool.setPatchStatus('${id}', 'approved')">Confirm</button>
                    <button class="action-button small secondary" onclick="MergerTool.setPatchStatus('${id}', 'rejected')">Ignore</button>
                </div>`;
        } else {
            // INSERT
            let locationOpts = `<option value="bottom" selected>Bottom</option><option value="top">Top</option><option value="line">Line #</option>`;
            this.globalSourceSegments.forEach(seg => {
                if (seg.name) locationOpts += `<option value="${seg.endIndex}">After ${seg.name}</option>`;
            });
            ...
            bodyHtml = `
                <div class="code-compare-container insert-mode">
                    <div class="code-pane insert-only">
                        <div class="line-numbers" id="ln-${id}">${this.genLineNums(newSeg.content, 1)}</div>
                        <div class="code-content" id="edit-${id}" contenteditable="true" spellcheck="false">${this.escapeHtml(newSeg.content)}</div>
                    </div>
                </div>`;
            headerRightHtml = `
                <div style="display:flex; gap:5px; align-items:center;">
                    <select id="loc-${id}" class="input-dark" style="padding:2px;" onchange="MergerTool.toggleLineInput('${id}')">${locationOpts}</select>
                    <input type="number" id="line-input-${id}" class="input-dark" style="width:50px; display:none;" placeholder="#">
                </div>
                <div class="diff-btn-row" style="margin-top:5px;">
                    <button class="action-button small" style="background:#007acc" onclick="MergerTool.setPatchStatus('${id}', 'approved')">Insert</button>
                    <button class="action-button small secondary" onclick="MergerTool.setPatchStatus('${id}', 'rejected')">Ignore</button>
                </div>`;
        }
        ...
        card.innerHTML = `
            <div class="diff-header">
                <div class="diff-info">${headerLeftHtml}</div>
                <div class="diff-actions">${headerRightHtml}</div>
            </div>
            ${bodyHtml}
        `;
        document.getElementById('mergerDiffContainer').appendChild(card);

        // Bind editor input event manually to avoid global scope clutter
        const editor = card.querySelector(`#edit-${id}`);
        if(editor) {
            editor.addEventListener('input', () => {
                const ln = card.querySelector(`#ln-${id}`);
                if(ln) ln.innerHTML = this.genLineNums(editor.innerText, 1);
            });
        }
    },
    ...
    toggleLineInput(id) {
        ...
        input.style.display = (select.value === 'line') ? 'inline-block' : 'none';
        if (select.value === 'line') input.focus();
    },

    setPatchStatus(id, status) {
        const patch = this.proposedPatches.find(p => p.id == id);
        if (!patch) return;
        patch.status = status;
    ...
    getIndexFromLineNumber(text, lineNum) {
        const lines = text.split('\n');
        if (lineNum > lines.length) return text.length; 
        if (lineNum <= 1) return 0;
        const preceding = lines.slice(0, lineNum - 1).join('\n');
        return preceding.length + 1;
    },

    applyPatches() {
        const validPatches = this.proposedPatches.filter(p => p.status === 'approved');
        let operations = [];
        ...
        validPatches.forEach(p => {
            ...
            if (p.type === 'replace') {
                ...
                let insertIndex;
                if (locValue === 'top') insertIndex = 0;
                else if (locValue === 'bottom') insertIndex = Infinity;
                else if (locValue === 'line') {
                    const lineInput = document.getElementById(`line-input-${p.id}`);
                    let ln = parseInt(lineInput.value) || 0;
                    if(ln < 1) ln = 1;
                    insertIndex = this.getIndexFromLineNumber(this.rawOriginalSource, ln);
                } else {
        ...
        operations = operations.filter(op => {
            if (op.index === Infinity) { bottomAppends += op.content; return false; }
            return true;
        });
        operations.sort((a, b) => b.index - a.index);

        let tempCode = this.rawOriginalSource;
        ...
        this.sourceCode = tempCode;
        return tempCode;
    }
};

// Make MergerTool global for inline HTML events
window.MergerTool = MergerTool;

function renderMergerTab() {
    const tabContainer = document.getElementById('merger');
    if (!tabContainer) return;
    
    // Check if already rendered (fixing issue with HTML comments in empty div)
    if (tabContainer.querySelector('.merger-container')) return;

    tabContainer.innerHTML = `
        <div class="merger-container">
            <div class="merger-controls">
                <div style="flex:1;">
                    <label style="display:block; font-size:0.8em; margin-bottom:4px; color:#aaa;">1. Load Source File (.js/.ts/.html)</label>
                    <div style="display:flex; gap:10px;">
                        <input type="file" id="mergerFileInput" class="input-dark" accept=".js,.jsx,.ts,.tsx,.html" style="flex:1;">
                        <button id="mergerLoadFromCodeBtn" class="action-button secondary" title="Load content from Full Code Editor">Load from Editor</button>
                    </div>
                </div>
                <div style="display:flex; gap:10px;">
                    <button id="mergerAnalyzeBtn" class="action-button" disabled>Analyze & Diff</button>
                    <button id="mergerDownloadBtn" class="action-button success" disabled>Apply & Download</button>
                </div>
            </div>
            <div class="merger-workspace">
                <div class="merger-column">
                    <h3>2. Paste AI Code Segment</h3>
                    <textarea id="mergerAiInput" class="merger-textarea" placeholder="// Paste code here...\nfunction newFeature() {\n   // This can be edited later\n}"></textarea>
                </div>
                <div class="merger-column">
                    <h3>3. Review, Edit & Merge</h3>
                    <div id="mergerDiffContainer" class="diff-list">
    ...
    // Event Listeners for Merger Tab
    const fileInput = document.getElementById('mergerFileInput');
    const loadFromCodeBtn = document.getElementById('mergerLoadFromCodeBtn');
    const aiInput = document.getElementById('mergerAiInput');
    const analyzeBtn = document.getElementById('mergerAnalyzeBtn');
    const downloadBtn = document.getElementById('mergerDownloadBtn');
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        const reader = new FileReader();
        reader.onload = (ev) => {
            MergerTool.loadSource(ev.target.result, file.name);
        };
        reader.readAsText(file);
    });

    loadFromCodeBtn.addEventListener('click', () => {
        const currentCode = fullCodeEditor ? fullCodeEditor.value : '';
        if(!currentCode) {
            alert("No code in the Full Code Editor to load.");
            return;
        }
        MergerTool.loadSource(currentCode, "current-project-code.html");
        alert("Loaded code from Full Code Editor.");
    });

    aiInput.addEventListener('input', () => MergerTool.checkReady());

    analyzeBtn.addEventListener('click', () => {
        MergerTool.performAnalysis(aiInput.value);
    });

    downloadBtn.addEventListener('click', () => {
        const finalCode = MergerTool.applyPatches();
        const blob = new Blob([finalCode], { type: 'text/plain' });
        triggerBlobDownload(blob, MergerTool.sourceFilename || 'merged.js');
    });
}
