...
async function restoreSession() {
    ...
    try {
        ...
        if (type === 'cloud') {
            // For cloud, we need to be logged in. currentUser is set by checkLoggedInState previously.
            if (!currentUser) {
                console.log("Session restore skipped: User not logged in for cloud project.");
                clearSessionMetadata(); // Clear stale data
                return;
            }
        ...
        if (restoredTree) {
            ...
            shareProjectButton.disabled = (type !== 'cloud');
            openAiStructureModalButton.disabled = (type !== 'cloud');
            updateSaveButtonStates();
