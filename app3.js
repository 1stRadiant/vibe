...
// --- START OF CHANGE: Local Storage Project Persistence ---
const LOCAL_PROJECT_PREFIX = '_vibe_local_project_';

const localApi = {
    async listProjects() {
        return Promise.resolve(
            Object.keys(localStorage)
                .filter(key => key.startsWith(LOCAL_PROJECT_PREFIX))
                .map(key => key.replace(LOCAL_PROJECT_PREFIX, ''))
        );
    },
    async saveProject(projectId, projectData) {
        try {
            const compressed = compressProjectData(projectData);
            localStorage.setItem(`${LOCAL_PROJECT_PREFIX}${projectId}`, compressed);
            console.log(`Locally saved project '${projectId}'.`);
            return Promise.resolve();
        } catch (e) {
            console.error(`Failed to save local project '${projectId}':`, e);
            return Promise.reject(e);
        }
    },
    async loadProject(projectId) {
        const data = localStorage.getItem(`${LOCAL_PROJECT_PREFIX}${projectId}`);
        if (data) {
            return Promise.resolve(data);
        } else {
            return Promise.reject(new Error(`Local project '${projectId}' not found.`));
        }
    },
    async deleteProject(projectId) {
        localStorage.removeItem(`${LOCAL_PROJECT_PREFIX}${projectId}`);
        console.log(`Locally deleted project '${projectId}'.`);
        return Promise.resolve();
    }
};
// --- END OF CHANGE ---
