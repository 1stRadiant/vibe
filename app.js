...
// Context Import/Export elements
...
// Context Component Modal elements
...
// AI generation elements for component modal
...
// Hidden fields for inspector-based adding
...
const inspectorElementInfo = document.getElementById('inspector-element-info');
const inspectorStylesContainer = document.getElementById('inspector-styles-container');


// START OF FIX: Replaced inspector update logic with a more robust and complete version.
function updateStyleInVibeTree(cssNodeId, selector, property, value, remove = false) {
    console.log(`Updating style: ${cssNodeId} -> ${selector} { ${property}: ${value} } (Remove: ${remove})`);
    const cssNode = findNodeById(cssNodeId);
    if (!cssNode) {
        console.error("Cannot update style: CSS source node not found:", cssNodeId);
        return;
    }

    // Do not record history for empty property names, which can happen during creation.
    if (property) recordHistory(`CSS edit: ${property}`);

    let cssCode = cssNode.code || '';
    
    const safeSelector = selector.replace(/([.*+?^${}()|[\]\\])/g, '\\$1');
    const selectorRegex = new RegExp(`(${safeSelector}(?:\\s*\\/\\*.*?\\*\/)*?\\s*\\{[^}]*?\\})`, 'g');
    
    let ruleUpdated = false;
    cssCode = cssCode.replace(selectorRegex, (match) => {
        ruleUpdated = true;
        
        if (remove) {
            // Remove the property. Find `property: value;` and remove it.
            const propRemoveRegex = new RegExp(`\\s*${property}\\s*:\\s*[^;]+;?`, 'i');
            return match.replace(propRemoveRegex, '');
        } else {
            // Update or add the property
            const propUpdateRegex = new RegExp(`([;\\{\\s]${property}\\s*:\\s*)[^;]+(;?)`, 'i');
            if (propUpdateRegex.test(match)) {
                return match.replace(propUpdateRegex, `$1${value}$2`);
            } else {
                return match.replace(/(\s*})$/, `; ${property}: ${value};\n$1`);
            }
        }
    });

    if (!ruleUpdated && !remove) {
        cssCode += `\n\n${selector} {\n    ${property}: ${value};\n}`;
    }

    // Cleanup: remove double semicolons, semicolons right after an opening brace, or semicolons right before a closing brace.
    cssCode = cssCode.replace(/;(\s*;)+/g, ';').replace(/\{\s*;/g, '{ ').replace(/;\s*}/g, '}');

    cssNode.code = cssCode.trim();

    try {
        const iframeDoc = previewContainer.contentWindow.document;
        const styleTag = iframeDoc.querySelector(`style[data-vibe-css-node-id="${cssNodeId}"]`);
        if (styleTag) {
            styleTag.textContent = `\n${cssNode.code}\n`;
            console.log("Live-injected style update into iframe.");
        } else {
            console.warn("Could not find style tag for live update. Falling back to full refresh.");
            applyVibes();
        }
    } catch (e) {
        console.error("Error during live style injection:", e);
        applyVibes();
    }
...
function displayInInspector(data) {
    if (!inspectorPanel || !inspectorElementInfo || !inspectorStylesContainer) return;
    
    inspectorPanel.classList.add('visible');
    
    const classStr = data.classes.length > 0 ? `.${data.classes.join('.')}` : '';
    inspectorElementInfo.innerHTML = `
        <strong>&lt;${data.tagName.toLowerCase()}&gt;</strong>
        ${data.id ? `#${data.id}` : ''}${classStr}
        <br>
        <small>Vibe Node: <code>${data.nodeId}</code></small>
    `;

    inspectorStylesContainer.innerHTML = '';
    ...
    if (data.matchingRules && data.matchingRules.length > 0) {
        data.matchingRules.forEach(rule => {
            ...
            ruleBlock.className = 'css-rule-block';

            let propertiesHtml = '';
            for (const [prop, val] of Object.entries(rule.style)) {
                propertiesHtml += `
                    <div class="css-property">
                        <input type="checkbox" class="css-property-toggle" checked>
                        <span class="css-property-name" contenteditable="true">${prop}</span>:
                        <span class="css-property-value" contenteditable="true">${val}</span>;
                    </div>
                `;
            }
            ...
            ruleBlock.innerHTML = `
                <div class="css-rule-header">
                    <span class="css-rule-selector">${rule.selector}</span>
                    <span class="css-rule-source" data-css-node-id="${rule.sourceNodeId}">source</span>
                </div>
                <div class="css-properties" data-selector="${rule.selector}" data-css-node-id="${rule.sourceNodeId}">
                    ${propertiesHtml}
                    <button class="add-property-button action-button">+ Add</button>
                </div>
            `;
            inspectorStylesContainer.appendChild(ruleBlock);
        });
    } else {
        inspectorStylesContainer.innerHTML = '<p style="padding: 10px; color: #abb2bf;">No matching CSS rules found in Vibe nodes.</p>';
    }

    // Add event listeners for live editing
    const addLiveEditListeners = (scope) => {
        scope.querySelectorAll('.css-property-name, .css-property-value').forEach(el => {
            el.addEventListener('blur', (e) => {
                const propRow = e.target.closest('.css-property');
                const propContainer = e.target.closest('.css-properties');
                const nameEl = propRow.querySelector('.css-property-name');
                const valueEl = propRow.querySelector('.css-property-value');
                const toggle = propRow.querySelector('.css-property-toggle');

                if (!toggle.checked) {
                    toggle.checked = true;
                    propRow.classList.remove('disabled');
                }
                
                updateStyleInVibeTree(
                    propContainer.dataset.cssNodeId,
                    propContainer.dataset.selector,
        ...
        scope.querySelectorAll('.css-property-toggle').forEach(el => {
            el.addEventListener('change', (e) => {
                ...
                updateStyleInVibeTree(
                    ...
                    !isEnabled // remove if unchecked
                );
                propRow.classList.toggle('disabled', !isEnabled);
            });
        });
    };

    addLiveEditListeners(inspectorStylesContainer);

    inspectorStylesContainer.querySelectorAll('.add-property-button').forEach(button => {
        button.addEventListener('click', (e) => {
            const propContainer = e.target.closest('.css-properties');
            const newPropRow = document.createElement('div');
            newPropRow.className = 'css-property';
            newPropRow.innerHTML = `
                <input type="checkbox" class="css-property-toggle" checked>
                <span class="css-property-name" contenteditable="true">property</span>:
                <span class="css-property-value" contenteditable="true">value</span>;
            `;
            propContainer.insertBefore(newPropRow, e.target);
            addLiveEditListeners(newPropRow);
            const nameEl = newPropRow.querySelector('.css-property-name');
            nameEl.focus();
            document.execCommand('selectAll', false, null);
        });
    });

    inspectorStylesContainer.querySelectorAll('.css-rule-source').forEach(el => {
        el.addEventListener('click', (e) => {
            const cssNodeId = e.target.dataset.cssNodeId;
            if (cssNodeId) {
                switchToTab('editor');
                const nodeEl = editorContainer.querySelector(`.vibe-node[data-node-id="${cssNodeId}"]`);
                if (nodeEl) {
                    nodeEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
                    nodeEl.classList.add('ai-search-highlight');
                    setTimeout(() => nodeEl.classList.remove('ai-search-highlight'), 2500);
                }
            }
        });
    });
...
async function callAI(systemPrompt, userPrompt, forJson = false, streamCallback = null, options = {}) {
    const { skipFileContext = false } = options;
    console.log('--- Calling AI ---');
    
    let fileContext = '';
    if (!skipFileContext && currentProjectId) {
        try {
...
function parseHtmlToVibeTree(fullCode) {
    ...
    scriptTags.forEach((scriptTag, index) => {
        if (!scriptTag.src && scriptTag.textContent.trim()) {
            // FIX: Removed dangerous Regex function extraction that was corrupting code.
            // Instead, we save the entire script block as a global javascript node.
            // This is safer for uploads and prevents syntax errors.
            jsNodes.push({
...
function generateFullCodeString(tree = vibeTree) {
    ...
    let cssTags = ''; // For separate style tags
    ...
    function traverse(node, currentTree) {
        switch (node.type) {
            case 'head':
                ...
                break;
            case 'css':
                cssTags += `<style data-vibe-css-node-id="${node.id}">\n${node.code}\n</style>\n`;
                break;
            case 'javascript':
                jsContent += node.code + '\n\n';
                break;
...
(function(){
    ...
    // --- Part 1.5: Inspector Data Gathering ---
    function getCssSourceNodeId(rule) {
        if (!rule.parentStyleSheet || !rule.parentStyleSheet.ownerNode) return null;
        return rule.parentStyleSheet.ownerNode.dataset.vibeCssNodeId || null;
    }
        ...
        while (currentEl && currentEl.nodeType === 1) { 
            try {
                for (const sheet of document.styleSheets) {
                    if (!sheet.ownerNode || !sheet.ownerNode.dataset.vibeCssNodeId) {
                        continue;
                    }
                    for (const rule of sheet.cssRules) {
                        if (currentEl.matches(rule.selectorText)) {
                            if (!seenSelectors.has(rule.cssText)) {
                                seenSelectors.add(rule.cssText);
                                const style = {};
                                ...
                                matchingRules.push({
                                    ...
                                    style: style,
                                    sourceNodeId: getCssSourceNodeId(rule)
                                });
                            }
                        }
                    }
    ...
    document.addEventListener('click',e=>{
        ...
        const targetInfo = getNodeId(e.target);
        if (targetInfo) {
            e.preventDefault();
            e.stopPropagation();
            ...
            const payload = {
                ...
                tagName: targetInfo.element.tagName,
                id: targetInfo.element.id,
                classes: [...targetInfo.element.classList],
                matchingRules: getMatchingCssRules(targetInfo.element)
            };
            window.parent.postMessage({type:'vibe-element-inspect', payload: payload}, '*');
            ...
            setTimeout(()=>el.classList.remove('__vibe-inspect-highlight-clicked'),500);
        }
    },true);
    ...
    function serializeArg(arg, depth = 0, seen = new WeakSet()) {
        ...
        const type = typeof arg;
        if (['string', 'number', 'boolean', 'symbol', 'bigint'].includes(type)) return { type, value: arg.toString(), preview: arg.toString() };
        if (type === 'function') {
            const signature = arg.toString().match(/^(async\\\\s+)?function\\\\s*\\\\*?\\\\s*([a-zA-Z0-9_$]+)?\\\\s*\\\\([^)]*\\\\)/)?.[0] || 'function()';
            return { type: 'function', value: signature, preview: 'ƒ' };
        }
        if (seen.has(arg)) return { type: 'string', value: '[Circular]', preview: '[Circular]' };
        if (arg instanceof HTMLElement) return { type: 'dom', tagName: arg.tagName, id: arg.id, classes: [...arg.classList], preview: '<...>' };
        if (depth > MAX_DEPTH) return { type: 'string', value: Array.isArray(arg) ? '[Array]' : '[Object]', preview: '...' };
        seen.add(arg);
        if (Array.isArray(arg)) return { type: 'array', value: arg.map(item => serializeArg(item, depth + 1, seen)), preview: '[]' };
        if (type === 'object') {
            const obj = {};
            for (const key in arg) { if (Object.prototype.hasOwnProperty.call(arg, key)) obj[key] = serializeArg(arg[key], depth + 1, seen); }
            return { type: 'object', value: obj, preview: '{}' };
        }
        return { type: 'string', value: String(arg), preview: String(arg) };
    }
    function reportError(err){ const serialized = { message: err.message, stack: err.stack, name: err.name, __isError: true }; window.parent.postMessage({type:'iframe-error',payload:serialized},'*'); }
    window.addEventListener('error', e => reportError(e.error || e.message));
    window.addEventListener('unhandledrejection', e => reportError(e.reason));
    const oConsole = {...window.console};
    Object.keys(oConsole).forEach(level => { window.console[level] = (...args) => { oConsole[level](...args); try { window.parent.postMessage({ type: 'iframe-console', level, payload: args.map(p => serializeArg(p)) }, '*') } catch(e) { oConsole.error('Vibe proxy error', e) } }});
    ...
    window.addEventListener('message',e=>{
        if(e.data.type==='toggle-inspect'){
            ...
            else if(hoverEl){ hoverEl.classList.remove('__vibe-inspect-highlight-hover'); hoverEl=null; }
        }
    });
})();
<\/script>`;
        ...
        // After reloading the iframe, its internal inspector state is reset to 'false'.
        // We must re-send the current state from the parent to re-enable it if necessary.
        if (inspectEnabled) { // 'inspectEnabled' is the parent window's global state
            // Use a small timeout to ensure the iframe's script has initialized.
            setTimeout(() => {
                try {
                    console.log("Re-syncing inspector state with reloaded iframe.");
                    previewContainer.contentWindow.postMessage({ type: 'toggle-inspect', enabled: true }, '*');
                } catch(e) {
                    console.error("Failed to re-sync inspector state:", e);
                }
...
/**
 * Processes a chat response that is expected to contain a full HTML code block.
 * It renders the code block with an "Apply Changes" button.
 * @param {HTMLElement} parentElement The message element to populate.
 * @param {string} fullResponse The full text response from the AI.
 */
function processFullCodeResponse(parentElement, fullResponse) {
    const codeMatch = fullResponse.match(/```(?:html)?\s*([\s\S]*?)\s*```/);

    parentElement.innerHTML = ''; // Clear placeholder content

    if (!codeMatch) {
        parentElement.textContent = fullResponse; // Render as plain text if no code found
        return;
    }

    const explanationText = fullResponse.substring(0, codeMatch.index).trim();
    if (explanationText) {
    ...
    applyButton.className = 'insert-code-button';
    applyButton.textContent = 'Apply Changes to Project';
    applyButton.addEventListener('click', async (e) => {
        if (!confirm('This will replace your entire project with the code above. Are you sure?')) {
            return;
        }
        ...
        await processCodeAndRefreshUI(codeContent);
    ...
    const afterText = fullResponse.substring(codeMatch.index + codeMatch[0].length).trim();
    if (afterText) {
        const p = document.createElement('p');
...
async function handleSendChatMessage() {
    ...
    const systemPrompt = chatSystemPromptInput.value.trim() || `You are an expert pair programmer. The user will provide you with the **full bundled source code of their entire website** and a request to change it. Your task is to analyze the user's request and modify the code. Respond with a brief explanation of your changes, followed by the **complete, updated HTML file content**. Do not provide snippets, diffs, or partial code. The full code must be enclosed in a single markdown code block like this: \`\`\`html ... complete file content ... \`\`\``;
    ...
    try {
        const fullBundledCode = generateFullCodeString();
        const augmentedUserPrompt = `Here is the full source code of my website. Analyze it and then apply the requested change.
...
User Request: "${userPrompt}"`;
        ...
        const fullResponse = await callAI(systemPrompt, augmentedUserPrompt, false, streamCallback, { skipFileContext: true });
        
        processFullCodeResponse(aiMessageElement, fullResponse);
...
function toggleInspectMode() {
    ...
    try {
        ...
        if (!inspectEnabled && inspectorPanel) {
...
window.addEventListener('message', (event) => {
    ...
    switch (data.type) {
        case 'vibe-element-inspect':
            if (data.payload) {
                openEditNodeModal(data.payload.nodeId);
                displayInInspector(data.payload);
            }
            break;
...
function switchToTab(tabId) {
    ...
    // Deactivate current tab and all active buttons
    ...
    document.querySelectorAll('.tab-button.active, .nav-link.active').forEach(btn => btn.classList.remove('active'));

    // Activate new tab and all corresponding buttons
...
function bindEventListeners() {
    ...
    if (sidebarSettingsButton) sidebarSettingsButton.addEventListener('click', () => settingsModal.style.display = 'block');
    if (inspectorCloseButton) inspectorCloseButton.addEventListener('click', () => inspectorPanel.classList.remove('visible'));
