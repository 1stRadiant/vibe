...
async function generateCompleteSubtree(parentNode, streamCallback = null) {
    ...
    **JAVASCRIPT MODULARITY (MANDATORY):**
    *   **Do NOT** put all JavaScript in one block.
    *   **Functions:** Create a separate node with \`type: "js-function"\` for **EACH** function. ID format: \`function-{verb}-{noun}\` (e.g., \`function-toggle-nav\`).
    *   **Variables:** Create a separate node with \`type: "declaration"\` for **EACH** global variable/constant. ID format: \`var-{name}\`.
    *   **Initialization:** Use \`type: "javascript"\` ONLY for event listeners and setup code that calls the functions.

    **DESIGN GUIDELINES:**
    1.  **Aesthetics:** Use modern design trends (clean typography, subtle shadows, rounded corners).
    2.  **CSS:** Write robust, scoped CSS. Use Flexbox and Grid.
    3.  **Content:** Generate realistic, engaging placeholder content.

    **TASK:** Generate a valid JSON array of "vibe nodes". The output MUST be a JSON array [] and nothing else.

${getVibeDbInstructionsForAI()}
${getImageGenerationInstructions()}

**JSON SCHEMA:**
1.  **Component Node Object:**
    *   id: string (kebab-case)
    *   type: "head" | "html" | "css" | "javascript" | "js-function" | "declaration"
    *   description: string
    *   code: string
    *   children: (Optional) array of nodes
    *   selector: (For html) string
    *   position: (For html) "beforeend" | "afterend"
`;
...
**JAVASCRIPT DECOMPOSITION RULES (CRITICAL):**
1.  **Explode the Script:** You must break down \`<script>\` tags into granular nodes. Do NOT keep large blocks of JS.
2.  **Extract Functions:** Identify every function definition (\`function x() {}\` or \`const x = () => {}\`). Create a SEPARATE node for **EACH** function with \`type: "js-function"\` and \`id: "function-name"\`.
3.  **Extract Declarations:** Identify top-level variables. Create a SEPARATE node for **EACH** with \`type: "declaration"\` and \`id: "var-name"\`.
4.  **Residual Logic:** Put remaining setup code (event listeners, init calls) into a \`type: "javascript"\` node with \`id: "init-scripts"\`.

**HTML/CSS RULES:**
-   Identify logical containers (header, section, footer) and create parent HTML nodes.
-   Extract CSS from <style> tags into \`css\` nodes.

**OUTPUT:** A single, valid JSON object representing the vibe tree.
`;
...
- You can understand "Vibe Shorthand" (e.g., @navbar) but you are NOT limited to it.

**JAVASCRIPT ARCHITECTURE RULES:**
- **NO MONOLITHS:** Never create a single huge JavaScript node.
- **Granularity:** Break logic down.
  - Create a separate node with \`type: "js-function"\` for **EACH** distinct function.
  - Create a separate node with \`type: "declaration"\` for **EACH** global variable.
  - Use \`type: "javascript"\` ONLY for glue code (event listeners, initialization).
