...
let nsDragStart = { mouseX:0, mouseY:0, offsetX:0, offsetY:0 };
let nsDragDistance = 0;
...
// Track total drag distance so we can distinguish a click from a drag
let nsDragDistance = 0;
// Last raw mouse position relative to the canvas element (not the page)
let nsLastMouse = { x: 0, y: 0 };

function setupCanvasEvents() {
  if (!nsCanvas) return;

  // ── mousedown: record the canvas-relative start position ──────────────────
  nsCanvas.addEventListener('mousedown', e => {
    e.preventDefault();
    nsDragging    = true;
    nsDragDistance = 0;
    nsDragStart   = {
      mouseX:  e.offsetX,          // px inside the canvas element
      mouseY:  e.offsetY,
      offsetX: nsOffsetX,           // world offset at drag start
      offsetY: nsOffsetY,
    };
    nsCanvas.style.cursor = 'grabbing';
  });

  // ── mousemove: pan OR hover ────────────────────────────────────────────────
  nsCanvas.addEventListener('mousemove', e => {
    nsLastMouse = { x: e.offsetX, y: e.offsetY };

    if (nsDragging) {
      const dx = e.offsetX - nsDragStart.mouseX;
      const dy = e.offsetY - nsDragStart.mouseY;
      nsDragDistance = Math.sqrt(dx * dx + dy * dy);
      // Apply delta on top of the offset that existed when drag started
      nsOffsetX = nsDragStart.offsetX + dx;
      nsOffsetY = nsDragStart.offsetY + dy;
    ...
    if (node) {
      ...
      const err = nsErrors[node.id];
      // Keep tooltip inside the canvas wrapper
      const wrapper = nsCanvas.parentElement;
      const maxLeft = wrapper ? wrapper.clientWidth  - 260 : e.offsetX + 16;
      const maxTop  = wrapper ? wrapper.clientHeight - 160 : e.offsetY + 8;
      tooltip.style.display = 'block';
      tooltip.style.left = Math.min(e.offsetX + 16, maxLeft) + 'px';
      tooltip.style.top  = Math.min(e.offsetY + 8,  maxTop)  + 'px';
      tooltip.innerHTML = `
  ...
  // ── mouseup: end drag ─────────────────────────────────────────────────────
  nsCanvas.addEventListener('mouseup', e => {
    const wasDrag = nsDragDistance > 4;
    nsDragging = false;
    nsCanvas.style.cursor = 'grab';

    if (!wasDrag) {
      // Genuine click — open modal if over a node
      const node = getNodeAtMouse(e);
      if (node) openNsFixModal(node);
    }
  });

  // ── mouseleave: cancel drag, hide tooltip ─────────────────────────────────
  ...
  nsCanvas.addEventListener('touchstart', e => {
  ...
  nsCanvas.addEventListener('touchmove', e => {
    if (!nsDragging || e.touches.length !== 1) return;
    e.preventDefault();
    const t = e.touches[0];
    const rect = nsCanvas.getBoundingClientRect();
    const dx = (t.clientX - rect.left) - nsDragStart.mouseX;
    const dy = (t.clientY - rect.top)  - nsDragStart.mouseY;
    nsDragDistance = Math.sqrt(dx * dx + dy * dy);
    nsOffsetX = nsDragStart.offsetX + dx;
    nsOffsetY = nsDragStart.offsetY + dy;
  }, { passive: false });

  nsCanvas.addEventListener('touchend', () => { nsDragging = false; });

  // ── scroll wheel zoom (zoom toward cursor position) ────────────────────────
  nsCanvas.addEventListener('wheel', e => {
    e.preventDefault();
    const zoomFactor = e.deltaY > 0 ? 0.9 : 1.1;
    const newScale   = Math.max(0.15, Math.min(5, nsScale * zoomFactor));

    // Zoom toward the mouse cursor rather than the canvas centre
    const mx = e.offsetX - nsCanvas.width  / 2;
    const my = e.offsetY - nsCanvas.height / 2;
    nsOffsetX = mx + (nsOffsetX - mx) * (newScale / nsScale);
    nsOffsetY = my + (nsOffsetY - my) * (newScale / nsScale);
    nsScale   = newScale;
  }, { passive: false });
}

function getNodeAtMouse(e) {
  // Convert canvas-element-relative px → world coordinates
  // The draw transform is: ctx.translate(W/2 + nsOffsetX, H/2 + nsOffsetY); ctx.scale(nsScale, nsScale)
  const mx = (e.offsetX - nsCanvas.width  / 2 - nsOffsetX) / nsScale;
  const my = (e.offsetY - nsCanvas.height / 2 - nsOffsetY) / nsScale;
  for (const node of nsNodes) {
