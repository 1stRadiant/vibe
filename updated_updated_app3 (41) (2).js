...
const VIBE_JSON_LIBRARY = {
  ...
  // Note: Add the full code for HTML/CSS/JS from your JSON file for each item.
};
...
async function runLiveView(userId, projectId) {
    try {
        ...
        // START OF CHANGE: Handle raw HTML projects in live view
        let fullHtml;
        if (projectTree && projectTree.type === 'raw-html-container') {
             fullHtml = projectTree.code || '';
             // Note: VibeDB will not be injected into raw HTML projects in live view.
             // This could be added later by parsing and injecting the script if needed.
        } else {
...
function saveComponentLibrary(library) {
    try {
        ...
        // Add this line to refresh dropdown when library changes
        updateShorthandDropdown();
    } catch (e) {
...
let firstNodeToSwapId = null; // For click-to-swap functionality

// START OF CHANGE: Auto Pilot Event System State
...
let activeAutoPilotListeners = []; // Stores cleanup functions for active listeners
// END OF CHANGE
...
// MODIFIED: Updated to use reverse logging (prepend)
function renderConsoleMessage(level, args) {
    if (!consoleOutput) return;

    // START OF CHANGE: Emit Runtime Error Event
    if (level === 'error') {
    ...
    // START OF CHANGE: Reverse logging - Prepend message
    ...
    consoleOutput.scrollTop = 0; // Force view to top
    // END OF CHANGE
}
...
function handleConsoleCommand(level, args) {
    switch (level) {
        ...
        case 'groupCollapsed':
            ...
            // START OF CHANGE: Reverse logging for groups
            if (currentLogGroup.firstChild) {
...
async function callAI(systemPrompt, userPrompt, forJson = false, streamCallback = null) {
    ...
    // NEW: Inject full library code so AI can perform "Overrides"
    const components = listComponents();
    let libraryContext = "\n\n--- COMPONENT LIBRARY SOURCE CODE (FOR @ REFERENCES) ---\n";
    components.forEach(comp => {
...
function addEventListeners() {
    ...
    // START OF CHANGE: Replace drag-and-drop with click-to-swap
...
// MODIFIED: logToAgent now logs messages upwards (prepend) instead of downwards (append)
function logToAgent(message, type = 'info') {
    // START OF CHANGE: Emit Agent Log Event
    ...
    // START OF CHANGE: Prepend message to show newest at top
    if (agentOutput.firstChild) {
        agentOutput.insertBefore(msgEl, agentOutput.firstChild);
    } else {
        agentOutput.appendChild(msgEl);
    }
    agentOutput.scrollTop = 0; // Force view to top
    // END OF CHANGE
}

function getAgentSystemPrompt() {
    return `You are an expert AI developer agent that utilizes "Vibe Shorthand" to bridge natural language and structured code.

**THE VIBE SHORTHAND SYNTAX:**
1. **@component-id**: Direct invocation of a component from the user's library. You MUST use the provided library code as the source of truth for this node.
2. **> tag**: Defines a raw HTML container (e.g., "> section" creates a section node).
3. **Overrides:**: Defines property/content replacements for the preceding @component or > tag. Use key-value pairs.
4. **Indentation / numbering**: Indicates DOM hierarchy.

**CORE DIRECTIVE:**
When a user uses Shorthand, translate it into "actions" (create or update). 
- If "Overrides" are provided (e.g., Brand: "Lumiere"), you MUST modify the 'code' string of the library component to replace the corresponding placeholder or text.
- Maintain the hierarchy described by the shorthand.

**INTERACTIVE CLARIFICATION:**
If a request is ambiguous, return ONLY: { "question": "Your question here." }.

**OUTPUT FORMAT:**
Return ONLY a single, valid JSON object:
{
  "plan": "Explain how you mapped the shorthand to the hierarchy and handled overrides.",
  "actions": [
    {
...
// START OF CHANGE: New Auto Pilot Helper Functions

/**
 * Registers a background listener that injects a task when an event fires.
 */
function registerAutoPilotListener(eventName, taskDescription) {
    ...
    const listener = (e) => {
        // Prevent infinite checks if the task itself causes the event
    ...
    // Store for potential cleanup (not fully implemented in UI but good practice)
...
/**
 * Inserts text at the current cursor position in the Agent Input.
 */
...
/**
 * Renders the Trigger selection UI above the input.
 */
function renderAutoPilotTriggers() {
    // Fallback if container structure is different, we look for agentPromptInput parent
...
function handleAddTaskToQueue() {
    ...
    // START OF CHANGE: Check for "WHEN ... DO ..." syntax
...
async function processNextTaskInQueue() {
    if (!isTaskQueueRunning || taskQueue.length === 0) {
        ...
        // START OF CHANGE: Emit Queue Complete Event
...
/**
 * NEW: Displays a specialized UI for AI questions with quick response buttons.
 * @param {string} questionText - The question from the AI.
 */
function renderAgentQuestionUI(questionText) {
    ...
    `;
    logToAgent(questionHtml, 'plan'); // 'plan' style is usually prominent
}

/**
 * NEW: Handles clicks from the quick response buttons.
 * @param {string} replyText - The text to send back to the AI.
 */
function handleQuickReply(replyText) {
    if (!waitingForAgentConfirmation) return;
    
    // Populate the input and submit it to simulate user typing
    if (agentPromptInput) {
        agentPromptInput.value = replyText;
        handleAddTaskToQueue(); // This function handles the reply logic
    }
    
    // Disable buttons in the log to prevent double-clicking
...
async function executeSingleTask(promptContext) {
    ...
    try {
        ...
        if (agentDecision.question) {
            // MODIFIED: Use new UI renderer instead of generic log
...
function bindEventListeners() {
    ...
    // START OF CHANGE: Initialize Auto Pilot Triggers UI
    if (agentPromptInput) {
...
function resetToStartPage() {
    ...
    // Agent Output Reset (handled by prepend logic now)
    if(agentOutput) {
        agentOutput.innerHTML = ''; 
    ...
    // START OF CHANGE: Clear Auto Pilot Listeners
...
async function updateNodeByDescription(nodeId, newDescription, buttonEl = null) {
    ...
    try {
        if (node.type === 'container') {
            ...
            // Check if the AI wants to ask a question
            if (agentDecision.question) {
                // Since this is a modal update, we can't easily use the Agent UI.
                // We fallback to an alert for this specific context.
...
initializeApiSettings();
    ...
    // Attach Suggestion Engine to relevant inputs
...
window.vibeAPI = {
    ...
    // Expose handleQuickReply for the new Agent Question UI buttons
...
const shorthandSelect = document.getElementById('component-shorthand-select');

const insertShorthandButton = document.getElementById('insert-shorthand-button');

function updateShorthandDropdown() {
    if (!shorthandSelect) return;
    const comps = listComponents();
    shorthandSelect.innerHTML = '<option value="">-- Insert Component Variable --</option>';
    
    comps.sort((a, b) => a.name.localeCompare(b.name)).forEach(comp => {
        const option = document.createElement('option');
        option.value = `@${comp.id}`;
        option.textContent = `${comp.name} (@${comp.id})`;
        shorthandSelect.appendChild(option);
    });
}

function handleInsertShorthand() {
    const variable = shorthandSelect.value;
    if (!variable) return;
    ...
    shorthandSelect.value = ""; // Reset dropdown
}

if (insertShorthandButton) insertShorthandButton.addEventListener('click', handleInsertShorthand);

if (shorthandSelect) shorthandSelect.addEventListener('change', handleInsertShorthand);
...
function initializeShorthandSystem() {
    ...
    dropdowns.forEach(select => {
        ...
        // Clear and keep placeholder
        const placeholder = select.options[0];
        select.innerHTML = '';
        select.appendChild(placeholder);

        // Sort by ID and add ONLY the ID to the dropdown text
        components.sort((a, b) => a.id.localeCompare(b.id)).forEach(comp => {
            const option = document.createElement('option');
            option.value = `@${comp.id}`;
            option.textContent = `@${comp.id}`; // Only show the ID
...
if (agentSel) {
    agentSel.addEventListener('change', () => {
        insertAtCursor(document.getElementById('agent-prompt-input'), agentSel.value);
        agentSel.value = ""; // Reset
    });
}

if (startSel) {
    startSel.addEventListener('change', () => {
        insertAtCursor(document.getElementById('project-prompt-input'), startSel.value);
        startSel.value = ""; // Reset
    });
}
