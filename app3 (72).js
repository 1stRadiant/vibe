...
function injectBridgeIntoApui() {
    ...
    // Attach the incoming message listener once
    if (!_apuiBridgeListenerAttached) {
...
// After every refreshAllUI, push latest tree to APUI if it's visible
const _origRefreshAllUI = refreshAllUI;
function refreshAllUI(...args) {
    _origRefreshAllUI(...args);
    try {
        const apuiTab = document.getElementById('apui');
        const frame   = document.getElementById('apui-frame');
        if (apuiTab?.classList.contains('active') && frame) {
            frame.contentWindow.postMessage({
                type: 'apui-init',
                vibeTree: JSON.parse(JSON.stringify(vibeTree))
            }, '*');
