...
function generateFullCodeString(tree, userId, projectId) {
    ...
    const buildHtmlRecursive = (nodes) => {
        ...
        htmlNodes.forEach(node => {
            ...
            // Add ID for inspector support if missing
            if (nodeCode.trim().startsWith('<') && !nodeCode.includes('data-vibe-node-id')) {
                nodeCode = nodeCode.replace(/<([a-zA-Z0-9\-]+)/, `<$1 data-vibe-node-id="${node.id}"`);
            }

            if (node.children && node.children.length > 0) {
                const innerHtml = buildHtmlRecursive(node.children);
                const template = document.createElement('template');
                template.innerHTML = nodeCode.trim();
                const content = template.content.firstElementChild;
                
                if (content) {
                    content.innerHTML = innerHtml;
                    currentHtml += content.outerHTML + '\n';
                } else {
                    currentHtml += nodeCode + innerHtml + '\n';
                }
            } else {
                currentHtml += nodeCode + '\n';
            }
        });
        return currentHtml;
    };
    ...
    function traverse(node) {
        ...
        if (node.type === 'css' && node.code) cssContent += node.code + '\n';
    ...
    const vibeDbScript = (userId && projectId) ? `
<script>
/* Vibe DB Connector */
(function() {
...
async function handleShareProject() {
    ...
    // REMOVED: Blocker for raw-html-container projects. 
    // runLiveView already contains the logic to handle projectTree.type === 'raw-html-container'.
    ...
    try {
        ...
        shareProjectButton.innerHTML = '<i class="bi bi-check-lg"></i> Link Copied!';
...
async function decomposeCodeIntoVibeTree(fullCode) {
    const systemPrompt = `You are a Code-to-Tree Deconstructor.
    
**TASK**: Convert the provided HTML/CSS/JS code into a "Vibe Tree" JSON.
**CRITICAL RULES**:
1. All CSS must be moved into nodes of type: "css".
2. All JavaScript inside <script> tags must be moved into nodes of type: "javascript".
3. The <body> content must be split into logical "html" nodes.
4. If an HTML element has children in the code, represent them as "children" in the JSON.
5. DO NOT leave <style> or <script> tags inside the "code" field of "html" nodes.
...
**JSON SCHEMA**:
{
  ...
  "type": "container",
  "children": [
    { "id": "meta", "type": "head", "code": "..." },
    { "id": "styles", "type": "css", "code": "..." },
    { "id": "main-nav", "type": "html", "code": "<nav>...</nav>", "children": [...] }
  ]
}`;

    const userPrompt = `Deconstruct this code:\n\n${fullCode}`;

    try {
        const rawResponse = await callAI(systemPrompt, userPrompt, true);
        const parsed = JSON.parse(rawResponse.replace(/|/g, ''));
        return (parsed.id === 'whole-page') ? parsed : parseHtmlToVibeTree(fullCode);
    } catch (e) {
        console.warn("AI Decomposition failed, using local parser fallback.");
...
async function processCodeAndRefreshUI(fullCode) {
    ...
    button.disabled = true;
    button.innerHTML = 'Analyzing Project... <div class="loading-spinner"></div>';
    showGlobalAgentLoader('AI is deconstructing your project into an editable tree...');
    
    try {
        recordHistory('Process into Vibe Tree');
        // We explicitly tell the AI to extract everything into a structured tree
        const newVibeTree = await decomposeCodeIntoVibeTree(fullCode);
        
        // Ensure we don't end up with an empty tree if AI fails
        if (!newVibeTree || !newVibeTree.children || newVibeTree.children.length === 0) {
            throw new Error("AI failed to generate a valid structure. Try again or check the console.");
        }
...
async function handleUpdateTreeFromCode(codeOverride = null) {
    ...
    // If we are in raw mode and the button says "Update from Code", we just save the raw text.
    // If the button says "Process into Vibe Tree" (handled via processCodeAndRefreshUI), 
    // we go straight to decomposition.
    if (vibeTree && vibeTree.type === 'raw-html-container') {
        recordHistory("Update raw HTML code");
        vibeTree.code = fullCode;
        applyVibes(); 
        autoSaveProject();
        console.log("Raw HTML updated.");
        return; 
    }

    // Standard Vibe Tree Update (for projects already structured)
    const button = updateTreeFromCodeButton;
    const originalButtonHtml = button.innerHTML;
    button.disabled = true;
    ...
    try {
        recordHistory('Update tree from code');
        const newVibeTree = await decomposeCodeIntoVibeTree(fullCode);
        vibeTree = newVibeTree;
        refreshAllUI();
        switchToTab('preview');
        autoSaveProject();
        if (codeOverride) alert("Code merged successfully!");
    } catch (error) {
...
**ROLE:**
You are a "Full Code Editor". You will be given the current state of a website project. Your job is to rewrite the ENTIRE HTML source code to satisfy the user's request.

**INPUT CONTEXT:**
1. **User Request:** What needs to be changed.
2. **Vibe Tree:** A JSON representation of the component structure. Use this ONLY for context to understand how the code is organized (e.g., seeing IDs, descriptions, and hierarchy).
3. **Current Code:** The actual HTML/CSS/JS of the project.

**OUTPUT FORMAT:**
Return ONLY a single, valid JSON object with the following structure:
{
  "plan": "A concise explanation of the changes you are making (e.g., 'Added a new contact section and updated the navbar links').",
  "code": "The COMPLETE, UPDATED HTML code for the entire project. Do not use placeholders. Do not return partial snippets. Return the full <!DOCTYPE html> document."
}

**RULES:**
1. Always return the full code, even if the change is small.
2. Ensure the code is valid HTML5.
3. Preserve existing functionality unless asked to change it.
4. If the Vibe Tree provides context about specific component IDs, try to maintain those IDs in the HTML tags to preserve structure, but you are free to refactor if necessary.
...
async function executeSingleTask(promptContext) {
    ...
    try {
        ...
        const userPrompt = `User Request History:\n"${promptContext}"\n\nFull Vibe Tree (For Context Only):\n\`\`\`json\n${fullTreeString}\n\`\`\`\n\nFull Current Code (To Edit):\n\`\`\`html\n${fullCurrentCode}\n\`\`\``;
        ...
        try {
            ...
            // Fallback: If AI fails to return JSON, check if it returned just code in a block
            const codeMatch = rawResponse.match(/```html\s*([\s\S]*?)\s*```/);
            if (codeMatch) {
                agentDecision = {
                    plan: "AI returned raw code. Processing update...",
                    code: codeMatch[1]
                };
            } else {
                throw new Error("AI response was not valid JSON or recognized code block.");
            }
        }
        ...
        // Await the execution because it now involves async decomposition
        await executeAgentPlan(agentDecision, logToAgent);
...
async function executeAgentPlan(agentDecision, agentLogger) {
    if (!agentDecision.code) {
        console.error("AI did not return a 'code' field.", agentDecision);
        throw new Error("AI did not return the updated code. Please try again.");
    }

    const planText = agentDecision.plan || "Updating project code...";
    agentLogger(`<strong>Plan:</strong> ${planText}`, 'plan');

    // 1. Record History before the massive change
    recordHistory('Agent Full Code Update');

    // 2. Process the code
    // We reuse the logic from the "Full Code" tab (processCodeAndRefreshUI)
    // but we extract the logic here to avoid UI button dependency.
    
    agentLogger(`<strong>System:</strong> Parsing new code and reconstructing Vibe Tree...`, 'info');

    try {
        let newVibeTree;
        
        // Check if we are in raw mode or tree mode
        if (vibeTree && vibeTree.type === 'raw-html-container') {
             vibeTree.code = agentDecision.code;
             newVibeTree = vibeTree; // Structure stays as raw container
        } else {
            // Use the AI decomposition to rebuild the tree from the new code
            newVibeTree = await decomposeCodeIntoVibeTree(agentDecision.code);
        }

        vibeTree = newVibeTree;
        refreshAllUI();
        
        agentLogger(`<strong>Success:</strong> Project updated.`, 'success');
        
    } catch (error) {
        console.error("Agent execution failed during code decomposition:", error);
        agentLogger(`<strong>Error:</strong> Failed to process the new code: ${error.message}`, 'error');
        throw error; // Re-throw so the task queue knows it failed
    }
}
...
function setCanvasSize(size) {
    ...
    document.querySelectorAll('.device-toggles button').forEach(btn => btn.classList.remove('active'));
...
function initNoCodeEditor() {
    ...
    // Listen for node selection from the canvas
    window.addEventListener('message', (event) => {
        if (event.data.type === 'vibe-node-click') {
            handleNoCodeNodeSelect(event.data.nodeId);
        }
    });

    const searchInput = document.getElementById('no-code-lib-search');
    if (searchInput) {
        searchInput.addEventListener('input', (e) => renderNoCodeLibrary(e.target.value));
    }
}

function refreshNoCodeCanvas() {
    const canvas = document.getElementById('no-code-canvas');
    if (!canvas) return;
    ...
    (function(){
        ...
        function getNodeId(el){const c=el.closest('[data-vibe-node-id]');return c?{nodeId:c.dataset.vibeNodeId,element:c}:null}
        const s=document.createElement('style');s.textContent=inspectorStyles;document.head.appendChild(s);
        document.addEventListener('mouseover',e=>{
            const t=getNodeId(e.target);
            if(t){
                if(hoverEl) hoverEl.classList.remove('__vibe-inspect-highlight-hover');
                hoverEl=t.element; hoverEl.classList.add('__vibe-inspect-highlight-hover');
            }
        });
        document.addEventListener('click',e=>{
            ...
            if(t){
                ...
                window.parent.postMessage({type:'vibe-node-click',nodeId:t.nodeId},'*');
            }
        },true);
    })();
    <\/script>`;
    ...
    const doc = canvas.contentWindow.document;
    doc.open();
    doc.write(html);
    doc.close();
}

function handleNoCodeNodeSelect(nodeId) {
    selectedNodeId = nodeId;
    const node = findNodeById(nodeId);
    const statusEl = document.getElementById('no-code-selection-status');
    const panel = document.getElementById('no-code-properties-panel');
    const cta = document.getElementById('no-code-ai-controls-cta');
    const genBtn = document.getElementById('visual-generate-controls-btn');

    if (!node) return;

    statusEl.innerHTML = `Editing: <strong>${nodeId}</strong> <span class="badge bg-secondary ms-2">${node.type}</span>`;
    panel.innerHTML = '';
    
    if (Array.isArray(node.controls) && node.controls.length > 0) {
        cta.style.display = 'none';
        node.controls.forEach(control => {
            const ctrlEl = renderControl(control, node.id);
            // Re-bind listeners for this specific panel
            ctrlEl.addEventListener('input', handleControlChange);
            ctrlEl.addEventListener('change', handleControlChange);
            panel.appendChild(ctrlEl);
        });
    } else {
        panel.innerHTML = `
            <div class="empty-selection-notice">
                <i class="bi bi-gear-wide-connected"></i>
                <p>No visual properties defined for this element.</p>
            </div>
        `;
        cta.style.display = 'block';
        genBtn.onclick = () => {
            aiControlsNodeIdInput.value = node.id;
            aiControlsPromptInput.value = "Create common styling controls for this element like colors, spacing, and text content.";
            handleExecuteGenerateControls().then(() => handleNoCodeNodeSelect(node.id));
        };
    }
}

function renderNoCodeLibrary(filter = '') {
    const container = document.getElementById('no-code-component-library');
    if (!container) return;
    container.innerHTML = '';
    ...
    filtered.forEach(comp => {
        const item = document.createElement('div');
        item.className = 'no-code-lib-item';
        item.innerHTML = `
            <span class="name">${comp.name}</span>
            <span class="desc">${comp.description || 'No description provided.'}</span>
        `;
        item.onclick = () => handleAddLibraryComponent(comp.id);
        container.appendChild(item);
    });
}

async function handleAddLibraryComponent(compId) {
    if (!currentProjectId) {
    ...
    recordHistory(`Add visual component: ${compId}`);
    
    const targetId = selectedNodeId || 'whole-page';
    const parentNode = findNodeById(targetId);
    
    if (!parentNode) return;

    const newNodeId = `${comp.id}-${Date.now() % 10000}`;
    const newNode = {
        id: newNodeId,
        type: 'html',
        description: `Visual instance of ${comp.name}`,
        code: comp.html.replace(/id="[^"]*"/g, `id="${newNodeId}"`),
        children: []
    };
    ...
    // Add CSS/JS if they exist and aren't already there
    if (comp.css) {
        vibeTree.children.push({
            id: `css-${newNodeId}`,
            type: 'css',
            description: `Styles for ${newNodeId}`,
            code: comp.css
        });
    }
    ...
    handleNoCodeNodeSelect(newNodeId);
}

function handleVisualAiEdit() {
    if (!selectedNodeId) {
        alert("Select an element on the canvas first to focus the AI edit.");
        return;
    }
    aiProjectEditNodeIdInput.value = selectedNodeId;
    aiProjectEditModalTitle.textContent = `AI Visual Edit: ${selectedNodeId}`;
    aiProjectEditModal.style.display = 'block';
    aiProjectEditPromptInput.focus();
}
